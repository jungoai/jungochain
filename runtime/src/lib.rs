...
parameter_types! {
    ...
    pub const RootEnterDuration : BlockNumber = 24 * HOURS;
    pub const RootExtendDuration: BlockNumber = 12 * HOURS;
...
impl GetVotingMembers<MemberCount> for GetSenateMemberCount {
    fn get_count() -> MemberCount {
        DelegateSenate::members().len() as u32
    }
}
impl Get<MemberCount> for GetSenateMemberCount {
    fn get() -> MemberCount {
        DelegateSenateMaxMembers::get()
    }
}
...
// We call our top K delegates membership Delegate Senate
#[allow(dead_code)]
type DelegateSenateInstance = pallet_membership::Instance2;
#[rustfmt::skip]
impl pallet_membership::Config<DelegateSenateInstance> for Runtime {
    type RuntimeEvent 			= RuntimeEvent;
    type AddOrigin 				= EnsureRoot<AccountId>;
    type RemoveOrigin 			= EnsureRoot<AccountId>;
    type SwapOrigin 			= EnsureRoot<AccountId>;
    ...
    type MaxMembers 			= DelegateSenateMaxMembers;
    type WeightInfo 			= pallet_membership::weights::SubstrateWeight<Runtime>;
}
parameter_types! {
    // Increase it, if you want add validator with stake to senate memebers
    pub const DelegateSenateMaxMembers: u32 = 0;
}

#[allow(dead_code)]
type ExpertSenateInstance = pallet_membership::Instance3;
#[rustfmt::skip]
impl pallet_membership::Config<ExpertSenateInstance> for Runtime {
    type RuntimeEvent 			= RuntimeEvent;
    type AddOrigin 				= EnsureRoot<AccountId>;
    type RemoveOrigin 			= EnsureRoot<AccountId>;
    type SwapOrigin 			= EnsureRoot<AccountId>;
    ...
    type MembershipInitialized 	= ();
    type MembershipChanged 		= ();
    type MaxMembers 			= ExpertSenateMaxMembers;
    type WeightInfo 			= pallet_membership::weights::SubstrateWeight<Runtime>;
}
parameter_types! {
    pub const ExpertSenateMaxMembers: u32 = 20;
}
...
impl InstanceFilter<RuntimeCall> for ProxyType {
    fn filter(&self, c: &RuntimeCall) -> bool {
        match self {
            ...
            ProxyType::Senate => matches!(c, RuntimeCall::DelegateSenate(..)),
            ProxyType::Governance => matches!(
                c,
                RuntimeCall::DelegateSenate(..)
                    | RuntimeCall::ExpertSenate(..)
                    | RuntimeCall::Triumvirate(..)
...
impl MemberManagement<AccountId> for ManageSenateMembers {
    fn add_delegate_member(account: &AccountId) -> DispatchResultWithPostInfo {
        ...
        DelegateSenate::add_member(RawOrigin::Root.into(), who)
    }

    fn remove_delegate_member(account: &AccountId) -> DispatchResultWithPostInfo {
        let who = Address::Id(account.clone());
        DelegateSenate::remove_member(RawOrigin::Root.into(), who)
    }
    ...
    fn swap_delegate_member(rm: &AccountId, add: &AccountId) -> DispatchResultWithPostInfo {
        ...
        DelegateSenate::swap_member(RawOrigin::Root.into(), remove, add)
    }

    fn delegate_members() -> Vec<AccountId> {
        DelegateSenate::members().into()
    }

    fn expert_members() -> Vec<AccountId> {
        ExpertSenate::members().into()
    }
...
construct_runtime!(
    pub struct Runtime
    {
        ...
        DelegateSenate      : pallet_membership::<Instance2>::{Pallet, Call, Storage, Event<T>, Config<T>} = 10,
        ExpertSenate        : pallet_membership::<Instance3>::{Pallet, Call, Storage, Event<T>, Config<T>} = 11,
        ...
        Registry    : pallet_registry       = 18,
        Commitments : pallet_commitments    = 19,
        AdminUtils  : pallet_admin_utils    = 20,
        SafeMode    : pallet_safe_mode      = 21,

        // Frontier
        Ethereum    : pallet_ethereum       = 22,
        EVM         : pallet_evm            = 23,
        EVMChainId  : pallet_evm_chain_id   = 24,
        DynamicFee  : pallet_dynamic_fee    = 25,
        BaseFee     : pallet_base_fee       = 26,
    }
);
