//! Jungochain pallet benchmarking.
#![allow(clippy::arithmetic_side_effects, clippy::unwrap_used)]
#![cfg(feature = "runtime-benchmarks")]

use crate::Pallet as Jungochain;
use crate::*;
...
benchmarks! {
  ...
  benchmark_register {
    ...
    Jungochain::<T>::init_new_network(netuid, tempo);
    Jungochain::<T>::set_network_registration_allowed(netuid, true);
    Jungochain::<T>::set_network_pow_registration_allowed(netuid, true);

    let block_number: u64 = Jungochain::<T>::get_current_block_as_u64();
    let (nonce, work): (u64, Vec<u8>) = Jungochain::<T>::create_work_for_block_number(
        netuid,
        block_number,
        3,
        &hotkey,
  ...
  benchmark_set_weights {
    ...
    Jungochain::<T>::init_new_network(netuid, tempo);
    Jungochain::<T>::set_max_allowed_uids( netuid, 4096 );

    Jungochain::<T>::set_network_registration_allowed( netuid, true );
    Jungochain::<T>::set_max_registrations_per_block( netuid, 4096 );
    Jungochain::<T>::set_target_registrations_per_interval( netuid, 4096 );
    ...
    for id in 0..4096_u16 {
      ...
      Jungochain::<T>::set_burn(netuid, 1);
      let amount_to_be_staked = 1000000u32.into();
      Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), amount_to_be_staked);

      Jungochain::<T>::do_burned_registration(RawOrigin::Signed(coldkey.clone()).into(), netuid, hotkey.clone())?;

      let uid = Jungochain::<T>::get_uid_for_net_and_hotkey(netuid, &hotkey.clone()).unwrap();
      Jungochain::<T>::set_validator_permit_for_uid(netuid, uid, true);
      dests.push(id);
      weights.push(id);
    }
  ...
  benchmark_become_delegate {
    ...
    Jungochain::<T>::set_network_registration_allowed( netuid, true);
    assert_eq!(Jungochain::<T>::get_max_allowed_uids(netuid), 4096);
    ...
    assert_ok!(Jungochain::<T>::do_burned_registration(RawOrigin::Signed(coldkey.clone()).into(), netuid, hotkey.clone()));
  ...
  benchmark_add_stake {
    ...
    Jungochain::<T>::set_target_stakes_per_interval(100);
  ...
  benchmark_remove_stake{
    ...
    Jungochain::<T>::increase_total_stake(1_000_000_000_000);
    ...
    Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), wallet_bal);

    assert_ok!(Jungochain::<T>::do_burned_registration(RawOrigin::Signed(coldkey.clone()).into(), netuid, hotkey.clone()));
    assert_ok!(Jungochain::<T>::do_become_delegate(RawOrigin::Signed(coldkey.clone()).into(), hotkey.clone(), Jungochain::<T>::get_default_delegate_take()));

      // Stake 10% of our current total staked TAO
      let u64_staked_amt = 100_000_000_000;
    Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), u64_staked_amt);

    assert_ok!( Jungochain::<T>::add_stake(RawOrigin::Signed( coldkey.clone() ).into() , hotkey.clone(), u64_staked_amt));
  ...
  benchmark_serve_axon{
    ...
    Jungochain::<T>::set_burn(netuid, 1);
    let amount_to_be_staked = 1000000u32.into();
    Jungochain::<T>::add_balance_to_coldkey_account(&caller.clone(), amount_to_be_staked);

    assert_ok!(Jungochain::<T>::do_burned_registration(caller_origin.clone(), netuid, caller.clone()));

    Jungochain::<T>::set_serving_rate_limit(netuid, 0);
  ...
  */
  ...
  benchmark_register_network {
    ...
    Jungochain::<T>::set_network_rate_limit(1);
  ...
  benchmark_dissolve_network {
    ...
    Jungochain::<T>::set_network_rate_limit(0);

    let amount: u64 = 1;
    let amount_to_be_staked = 100_000_000_000_000u64;
    Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), amount_to_be_staked);
    assert_ok!(Jungochain::<T>::register_network(RawOrigin::Signed(coldkey.clone()).into()));
  ...
  //   let netuid = 1u16;
  //   Jungochain::<T>::init_new_network(netuid, 100);
  //   Jungochain::<T>::set_min_burn(netuid, 1);
  //   Jungochain::<T>::set_max_burn(netuid, 1);
  //   Jungochain::<T>::set_target_registrations_per_interval(netuid, 256);
  //   Jungochain::<T>::set_max_registrations_per_block(netuid, 256);

  //   Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), 10_000_000_000u64);
  //   assert_ok!(Jungochain::<T>::burned_register(RawOrigin::Signed(coldkey.clone()).into(), netuid, old_hotkey.clone()));
  //   assert_ok!(Jungochain::<T>::become_delegate(RawOrigin::Signed(coldkey.clone()).into(), old_hotkey.clone()));

  //   let max_uids = Jungochain::<T>::get_max_allowed_uids(netuid) as u32;
  //   for i in 0..max_uids - 1 {
  //       let coldkey: T::AccountId = account("Axon", 0, i);
  //       let hotkey: T::AccountId = account("Hotkey", 0, i);

  //       Jungochain::<T>::add_balance_to_coldkey_account(&coldkey.clone(), 10_000_000_000u64);
  //       assert_ok!(Jungochain::<T>::burned_register(RawOrigin::Signed(coldkey.clone()).into(), netuid, hotkey));
  //       assert_ok!(Jungochain::<T>::add_stake(RawOrigin::Signed(coldkey).into(), old_hotkey.clone(), 1_000_000_000));
  //   }
  ...
  commit_weights {
    ...
    let result = Jungochain::<T>::register(
    ...
    Jungochain::<T>::set_validator_permit_for_uid(netuid, 0, true);
...
reveal_weights {
    ...
    let _ = Jungochain::<T>::commit_weights(<T as frame_system::Config>::RuntimeOrigin::from(RawOrigin::Signed(hotkey.clone())), netuid, commit_hash);
 ...
 benchmark_set_childkey_take {
  ...
  Jungochain::<T>::add_balance_to_coldkey_account(&coldkey, amount_to_be_staked);
  assert_ok!(Jungochain::<T>::do_burned_registration(RawOrigin::Signed(coldkey.clone()).into(), netuid, hotkey.clone()));
  ...
  swap_coldkey {
    ...
    let swap_cost = Jungochain::<T>::get_key_swap_cost();
    ...
    Jungochain::<T>::add_balance_to_coldkey_account(
        &old_coldkey,
...
batch_reveal_weights {
  ...
  Jungochain::<T>::set_network_registration_allowed(netuid, true);
  Jungochain::<T>::set_network_pow_registration_allowed(netuid, true);
  Jungochain::<T>::set_commit_reveal_weights_enabled(netuid, true);
  Jungochain::<T>::set_weights_set_rate_limit(netuid, 0); // Disable rate limiting for benchmarking
  ...
  assert_ok!(Jungochain::<T>::register(
      origin.clone(),
      netuid,
      block_number,
      nonce,
  ...
  for i in 0..num_commits {
      ...
      assert_ok!(Jungochain::<T>::commit_weights(
