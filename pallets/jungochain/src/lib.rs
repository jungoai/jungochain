...
pub mod pallet {
    ...
    /// ==== Staking Variables ====
    /// ============================
    /// The Jungochain [`TotalIssuance`] represents the total issuance of tokens on the Bittensor network.
    ///
...
#[freeze_struct("4a50f62a93647295")]
#[derive(Encode, Decode, Clone, Eq, PartialEq, TypeInfo)]
pub struct JungochainSignedExtension<T: Config + Send + Sync + TypeInfo>(pub PhantomData<T>);

impl<T: Config + Send + Sync + TypeInfo> Default for JungochainSignedExtension<T>
where
...
impl<T: Config + Send + Sync + TypeInfo> JungochainSignedExtension<T>
where
    <T as frame_system::Config>::RuntimeCall:
        Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
    <T as frame_system::Config>::RuntimeCall: IsSubType<Call<T>>,
...
impl<T: Config + Send + Sync + TypeInfo> sp_std::fmt::Debug for JungochainSignedExtension<T> {
    fn fmt(&self, f: &mut sp_std::fmt::Formatter) -> sp_std::fmt::Result {
        write!(f, "JungochainSignedExtension")
    }
}

impl<T: Config + Send + Sync + TypeInfo + pallet_balances::Config> SignedExtension
    for JungochainSignedExtension<T>
where
    <T as frame_system::Config>::RuntimeCall:
        Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
    <T as frame_system::Config>::RuntimeCall: IsSubType<Call<T>>,
...
{
    const IDENTIFIER: &'static str = "JungochainSignedExtension";
