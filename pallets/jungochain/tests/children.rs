...
mod mock;
use pallet_jungochain::{utils::rate_limiting::TransactionType, *};
use sp_core::U256;
...
fn test_do_set_child_singular_success() {
    new_test_ext(1).execute_with(|| {
        ...
        // Set child
        assert_ok!(JungochainModule::do_set_children(
        ...
        let children = JungochainModule::get_children(&hotkey, netuid);
...
fn test_do_set_child_singular_network_does_not_exist() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_err!(
            JungochainModule::do_set_children(
                RuntimeOrigin::signed(coldkey),
                hotkey,
                netuid,
                vec![(proportion, child)]
...
fn test_do_set_child_singular_root_network() {
    new_test_ext(1).execute_with(|| {
        ...
        let hotkey = U256::from(2);
        let child = U256::from(3);
        let netuid: u16 = JungochainModule::get_root_netuid(); // Root network
        let proportion: u64 = 1000;
...
fn test_do_set_child_singular_old_children_cleanup() {
    new_test_ext(1).execute_with(|| {
        ...
        let old_child_parents = JungochainModule::get_parents(&old_child, netuid);
        assert!(old_child_parents.is_empty());

        // Verify new child assignment
        let new_child_parents = JungochainModule::get_parents(&new_child, netuid);
...
fn test_do_set_child_singular_new_children_assignment() {
    new_test_ext(1).execute_with(|| {
        ...
        let parents = JungochainModule::get_parents(&child, netuid);
...
fn test_do_set_child_singular_multiple_children() {
    new_test_ext(1).execute_with(|| {
        ...
        let parents1 = JungochainModule::get_parents(&child1, netuid);
        assert!(parents1.is_empty()); // Old child should be removed

        let parents2 = JungochainModule::get_parents(&child2, netuid);
...
fn test_add_singular_child() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::create_account_if_non_existent(&coldkey, &hotkey);
        step_rate_limit(&TransactionType::SetChildren, netuid);
        assert_eq!(
            JungochainModule::do_set_children(
                RuntimeOrigin::signed(coldkey),
...
fn test_get_stake_for_hotkey_on_subnet() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey1, &parent, 1000);
        // Stake 1000 to parent from coldkey2
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey2, &parent, 1000);
        // Stake 1000 to child from coldkey1
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey1, &child, 1000);
        // Stake 1000 to child from coldkey2
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey2, &child, 1000);
        ...
        let parent_stake = JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        let child_stake = JungochainModule::get_stake_for_hotkey_on_subnet(&child, netuid);
...
fn test_do_set_children_multiple_old_children_cleanup() {
    new_test_ext(1).execute_with(|| {
        ...
        let new_child1_parents = JungochainModule::get_parents(&new_child1, netuid);
        assert_eq!(new_child1_parents, vec![(proportion, hotkey)]);

        let new_child2_parents = JungochainModule::get_parents(&new_child2, netuid);
...
fn test_do_set_children_multiple_overwrite_existing() {
    new_test_ext(1).execute_with(|| {
        ...
        let parents3 = JungochainModule::get_parents(&child3, netuid);
...
fn test_childkey_take_functionality() {
    new_test_ext(1).execute_with(|| {
        ...
        let default_take = JungochainModule::get_default_childkey_take();
        let min_take = JungochainModule::get_min_childkey_take();
        ...
        let new_take: u16 = JungochainModule::get_max_childkey_take() / 2; // 50% of max_take
        assert_ok!(JungochainModule::set_childkey_take(
            RuntimeOrigin::signed(coldkey),
            hotkey,
            netuid,
            new_take
        ...
        let stored_take = JungochainModule::get_childkey_take(&hotkey, netuid);
        log::info!("Stored take: {}", stored_take);
        assert_eq!(stored_take, new_take);

        // Test setting childkey take outside of allowed range
        let invalid_take: u16 = JungochainModule::get_max_childkey_take() + 1;
        assert_noop!(
            JungochainModule::set_childkey_take(
                RuntimeOrigin::signed(coldkey),
                hotkey,
                netuid,
                invalid_take
...
fn test_childkey_take_rate_limiting() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_tx_childkey_take_rate_limit(rate_limit);
        ...
        let log_rate_limit_info = || {
            let current_block = JungochainModule::get_current_block_as_u64();
            let last_block = JungochainModule::get_last_transaction_block(
                &hotkey,
                netuid,
                &TransactionType::SetChildkeyTake,
            );
            let passes = JungochainModule::passes_rate_limit_on_subnet(
                &TransactionType::SetChildkeyTake,
                &hotkey,
                netuid,
            );
            let limit = JungochainModule::get_rate_limit(&TransactionType::SetChildkeyTake, 0);
            log::info!(
        ...
        log_rate_limit_info();
        assert_noop!(
            JungochainModule::set_childkey_take(RuntimeOrigin::signed(coldkey), hotkey, netuid, 600),
        ...
        log_rate_limit_info();
        assert_noop!(
            JungochainModule::set_childkey_take(RuntimeOrigin::signed(coldkey), hotkey, netuid, 650),
            Error::<Test>::TxChildkeyTakeRateLimitExceeded
        );
        log_rate_limit_info();
    ...
    new_test_ext(1).execute_with(|| {
        ...
        for i in 1..NUM_NETWORKS {
            for j in (i + 1)..NUM_NETWORKS {
                let take_i = JungochainModule::get_childkey_take(&hotkey, i);
                let take_j = JungochainModule::get_childkey_take(&hotkey, j);
                assert_ne!(
                    take_i, take_j,
        ...
        let result =
            JungochainModule::set_childkey_take(RuntimeOrigin::signed(coldkey), hotkey, 1, 1100);
        assert_noop!(result, Error::<Test>::TxChildkeyTakeRateLimitExceeded);

        // Advance blocks to bypass rate limit
        run_to_block(JungochainModule::get_tx_childkey_take_rate_limit() + 1);
        ...
        let new_take = JungochainModule::get_childkey_take(&hotkey, 1);
...
fn test_do_revoke_children_multiple_partial_revocation() {
    new_test_ext(1).execute_with(|| {
        ...
        // Verify parents.
        let parents1 = JungochainModule::get_parents(&child3, netuid);
        assert!(parents1.is_empty());
        let parents1 = JungochainModule::get_parents(&child1, netuid);
...
fn test_get_network_max_stake() {
    new_test_ext(1).execute_with(|| {
        let netuid: u16 = 1;
        let default_max_stake = JungochainModule::get_network_max_stake(netuid);
        ...
        JungochainModule::set_network_max_stake(netuid, new_max_stake);

        // Check that the new value is retrieved correctly
        assert_eq!(
            JungochainModule::get_network_max_stake(netuid),
            new_max_stake
        );
    });
}
...
fn test_set_network_max_stake() {
    new_test_ext(1).execute_with(|| {
        let netuid: u16 = 1;
        let initial_max_stake = JungochainModule::get_network_max_stake(netuid);
...
fn test_set_network_max_stake_multiple_networks() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_network_max_stake(netuid1, max_stake1);
        JungochainModule::set_network_max_stake(netuid2, max_stake2);

        // Check that the values are set correctly for each network
        assert_eq!(JungochainModule::get_network_max_stake(netuid1), max_stake1);
        assert_eq!(JungochainModule::get_network_max_stake(netuid2), max_stake2);
        assert_ne!(
            JungochainModule::get_network_max_stake(netuid1),
            JungochainModule::get_network_max_stake(netuid2)
        );
    });
}
...
fn test_set_network_max_stake_update() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_network_max_stake(netuid, initial_max_stake);

        // Update the max stake value
        let updated_max_stake: u64 = 1_500_000;
        JungochainModule::set_network_max_stake(netuid, updated_max_stake);
...
fn test_children_stake_values() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add network and register hotkey
        add_network(netuid, 13, 0);
        JungochainModule::set_max_registrations_per_block(netuid, 4);
        JungochainModule::set_target_registrations_per_interval(netuid, 4);
        register_ok_neuron(netuid, hotkey, coldkey, 0);
        ...
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey, netuid),
            25_000_000_069_852
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child1, netuid),
            24_999_999_976_716
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child2, netuid),
            24_999_999_976_716
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child3, netuid),
            24_999_999_976_716
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child3, netuid)
...
fn test_get_parents_chain() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add network
        add_network(netuid, 13, 0);
        JungochainModule::set_max_registrations_per_block(netuid, 1000);
        JungochainModule::set_target_registrations_per_interval(netuid, 1000);
        ...
        for i in 1..num_keys {
            let parents = JungochainModule::get_parents(&hotkeys[i], netuid);
            log::info!(
        ...
        let root_parents = JungochainModule::get_parents(&hotkeys[0], netuid);
        log::info!(
        ...
        let last_hotkey_parents = JungochainModule::get_parents(&last_hotkey, netuid);
        log::info!(
...
fn test_childkey_single_parent_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey_parent, 1);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_child, 109_999);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_weight_setter, 1_000_000);
        ...
        JungochainModule::set_weights_set_rate_limit(netuid, 0);
        ...
        let version_key = JungochainModule::get_weights_version_key(netuid);
        assert_ok!(JungochainModule::set_weights(
            origin,
            netuid,
            uids,
            values,
        ...
        let parent_stake: u64 =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_parent, &parent);
        let parent_stake_on_subnet: u64 =
            JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        ...
        let child_stake: u64 =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_child, &child);
        let child_stake_on_subnet: u64 =
            JungochainModule::get_stake_for_hotkey_on_subnet(&child, netuid);
        ...
        let weight_setter_stake: u64 = JungochainModule::get_stake_for_coldkey_and_hotkey(
            &coldkey_weight_setter,
            &weight_setter,
        );
        let weight_setter_stake_on_subnet: u64 =
            JungochainModule::get_stake_for_hotkey_on_subnet(&weight_setter, netuid);
...
fn test_childkey_multiple_parents_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_max_registrations_per_block(netuid, 1000);
        JungochainModule::set_target_registrations_per_interval(netuid, 1000);
        JungochainModule::set_hotkey_emission_tempo(10);
        ...
        for (coldkey, hotkey, stake) in initial_stakes.iter() {
            JungochainModule::add_balance_to_coldkey_account(coldkey, *stake);
            register_ok_neuron(netuid, *hotkey, *coldkey, 0);
            JungochainModule::increase_stake_on_coldkey_hotkey_account(coldkey, hotkey, *stake);
        }
        ...
        for (coldkey, hotkey, name) in stakes.iter() {
            let stake = JungochainModule::get_stake_for_coldkey_and_hotkey(coldkey, hotkey);
            let stake_on_subnet = JungochainModule::get_stake_for_hotkey_on_subnet(hotkey, netuid);
            log::debug!(
        ...
        let parent1_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_parent1, &parent1);
        let parent2_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_parent2, &parent2);
        let child_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_child, &child);
        let weight_setter_stake = JungochainModule::get_stake_for_coldkey_and_hotkey(
            &coldkey_weight_setter,
            &weight_setter,
        );
...
fn test_parent_child_chain_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add initial stakes
        JungochainModule::add_balance_to_coldkey_account(&coldkey_a, 300_000);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_b, 100_000);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_c, 50_000);

        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_a, &hotkey_a, 300_000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_b, &hotkey_b, 100_000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_c, &hotkey_c, 50_000);
        ...
        let pending_emission_a = JungochainModule::get_pending_hotkey_emission(&hotkey_a);
        let pending_emission_b = JungochainModule::get_pending_hotkey_emission(&hotkey_b);
        let pending_emission_c = JungochainModule::get_pending_hotkey_emission(&hotkey_c);
        ...
        let stake_a: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey_a, netuid);
        let stake_b: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey_b, netuid);
        let stake_c: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey_c, netuid);
...
fn test_dynamic_parent_child_relationships() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add initial stakes
        JungochainModule::add_balance_to_coldkey_account(&coldkey_parent, 500_000);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_child1, 50_000);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_child2, 30_000);

        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_parent, &parent, 500_000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_child1, &child1, 50_000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey_child2, &child2, 30_000);
        ...
        // Run first epoch
        let hardcoded_emission: u64 = 1_000_000; // 1 million (adjust as needed)
        let hotkey_emission: Vec<(U256, u64, u64)> = JungochainModule::epoch(netuid, hardcoded_emission);

        // Process the hotkey emission results
        for (hotkey, mining_emission, validator_emission) in hotkey_emission {
            JungochainModule::accumulate_hotkey_emission(&hotkey, netuid, validator_emission, mining_emission);
        }
        ...
        let parent_stake: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        let child1_stake: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&child1, netuid);
        let child2_stake: u64 = JungochainModule::get_stake_for_hotkey_on_subnet(&child2, netuid);
        ...
        let parent_children: Vec<(u64, U256)> = JungochainModule::get_children(&parent, netuid);
        assert_eq!(
            parent_children,
            vec![(u64::MAX / 4, child1), (u64::MAX / 3, child2)],
        ...
        let child1_parents: Vec<(u64, U256)> = JungochainModule::get_parents(&child1, netuid);
        assert_eq!(
            child1_parents,
        ...
        let child2_parents: Vec<(u64, U256)> = JungochainModule::get_parents(&child2, netuid);
        assert_eq!(
            child2_parents,
...
fn test_get_stake_for_hotkey_on_subnet_basic() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &hotkey, 1000);
...
fn test_get_stake_for_hotkey_on_subnet_multiple_coldkeys() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey1, &hotkey, 1000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey2, &hotkey, 2000);
...
fn test_get_stake_for_hotkey_on_subnet_single_parent_child() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &parent, 1000);
...
fn test_get_stake_for_hotkey_on_subnet_multiple_parents_single_child() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &parent1, 1000);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &parent2, 2000);
        ...
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&parent1, netuid),
            501
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&parent2, netuid),
            1001
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child, netuid),
...
fn test_get_stake_for_hotkey_on_subnet_single_parent_multiple_children() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &parent, total_stake);
        ...
        let parent_stake = JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        let child1_stake = JungochainModule::get_stake_for_hotkey_on_subnet(&child1, netuid);
        let child2_stake = JungochainModule::get_stake_for_hotkey_on_subnet(&child2, netuid);
...
fn test_get_stake_for_hotkey_on_subnet_edge_cases() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_network_max_stake(netuid, network_max_stake);
...
fn test_get_stake_for_hotkey_on_subnet_complex_hierarchy() {
    new_test_ext(1).execute_with(|| {
        ...
        log::info!(
            "Grandchild stake: {}",
            JungochainModule::get_stake_for_hotkey_on_subnet(&grandchild, netuid)
        );
        ...
        log::info!(
            "Parent's children: {:?}",
            JungochainModule::get_children(&parent, netuid)
        );
        log::info!(
            "Child1's parents: {:?}",
            JungochainModule::get_parents(&child1, netuid)
        );
        log::info!(
            "Child2's parents: {:?}",
            JungochainModule::get_parents(&child2, netuid)
        );

        let parent_stake_1 = JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        let child1_stake_1 = JungochainModule::get_stake_for_hotkey_on_subnet(&child1, netuid);
        let child2_stake_1 = JungochainModule::get_stake_for_hotkey_on_subnet(&child2, netuid);
        ...
        log::info!(
            "Child1's children: {:?}",
            JungochainModule::get_children(&child1, netuid)
        );
        log::info!(
            "Grandchild's parents: {:?}",
            JungochainModule::get_parents(&grandchild, netuid)
        );

        let parent_stake_2 = JungochainModule::get_stake_for_hotkey_on_subnet(&parent, netuid);
        let child1_stake_2 = JungochainModule::get_stake_for_hotkey_on_subnet(&child1, netuid);
        let child2_stake_2 = JungochainModule::get_stake_for_hotkey_on_subnet(&child2, netuid);
...
fn test_get_stake_for_hotkey_on_subnet_multiple_networks() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey, netuid1),
            1000
        );
        assert_eq!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&hotkey, netuid2),
            1000
        );
    });
}
...
fn test_rank_trust_incentive_calculation_with_parent_child() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&parent_coldkey, initial_stake);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(
            &parent_coldkey,
            &parent_hotkey,
            initial_stake,
        );
        JungochainModule::add_balance_to_coldkey_account(&child_coldkey, initial_stake);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(
            &child_coldkey,
            &child_hotkey,
            initial_stake,
        ...
        for (hotkey, coldkey) in &other_validators {
            JungochainModule::add_balance_to_coldkey_account(coldkey, initial_stake);
            JungochainModule::increase_stake_on_coldkey_hotkey_account(
                coldkey,
                hotkey,
                initial_stake,
        ...
        let initial_emission = JungochainModule::epoch(netuid, rao_emission);
        ...
        let initial_child_rank: u16 = JungochainModule::get_rank_for_uid(netuid, 1);
        let initial_child_trust: u16 = JungochainModule::get_trust_for_uid(netuid, 1);
        let initial_child_incentive: u16 = JungochainModule::get_incentive_for_uid(netuid, 1);
        let initial_child_dividends: u16 = JungochainModule::get_dividends_for_uid(netuid, 1);
        ...
        // Run second epoch
        let final_emission = JungochainModule::epoch(netuid, rao_emission);
        ...
        let final_child_rank: u16 = JungochainModule::get_rank_for_uid(netuid, 1);
        let final_child_trust: u16 = JungochainModule::get_trust_for_uid(netuid, 1);
        let final_child_incentive: u16 = JungochainModule::get_incentive_for_uid(netuid, 1);
        let final_child_dividends: u16 = JungochainModule::get_dividends_for_uid(netuid, 1);
        ...
        for i in 0..5 {
            log::debug!(
                "Validator {} rank: {:?}",
                i,
                JungochainModule::get_rank_for_uid(netuid, i)
            );
        }
        ...
        for i in 2..5 {
            let other_rank: u16 = JungochainModule::get_rank_for_uid(netuid, i);
            let other_trust: u16 = JungochainModule::get_trust_for_uid(netuid, i);
            let other_incentive: u16 = JungochainModule::get_incentive_for_uid(netuid, i);
            let other_dividends: u16 = JungochainModule::get_dividends_for_uid(netuid, i);
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test children -- test_childkey_set_weights_single_parent --exact --nocapture
#[test]
fn test_childkey_set_weights_single_parent() {
    new_test_ext(1).execute_with(|| {
        ...
        // Register parent with minimal stake and child with high stake
        JungochainModule::add_balance_to_coldkey_account(&coldkey_parent, 1);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_child, stake_to_give_child + 10);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_weight_setter, 1_000_000);
        ...
        JungochainModule::set_weights_min_stake(stake_to_give_child * 5);

        // Check the child has less stake than required
        assert!(
            JungochainModule::get_stake_for_hotkey_on_subnet(&child, netuid)
                < JungochainModule::get_weights_min_stake()
        );

        // Check the child cannot set weights
        assert_noop!(
            JungochainModule::set_weights(
        ...
        assert!(!JungochainModule::check_weights_min_stake(&child, netuid));
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test children -- test_set_weights_no_parent --exact --nocapture
#[test]
fn test_set_weights_no_parent() {
    ...
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey, stake_to_give_child + 10);
        ...
        assert!(!JungochainModule::check_weights_min_stake(&hotkey, netuid));
...
fn test_childkey_take_drain() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_hotkey_emission_tempo(hotkey_tempo);
        JungochainModule::set_weights_set_rate_limit(netuid, 0);
        JungochainModule::set_max_allowed_validators(netuid, 2);
        step_block(subnet_tempo);
        pallet_jungochain::SubnetOwnerCut::<Test>::set(0);

        // Set 20% childkey take
        let max_take: u16 = 0xFFFF / 5;
        JungochainModule::set_max_childkey_take(max_take);
        assert_ok!(JungochainModule::set_childkey_take(
            RuntimeOrigin::signed(coldkey),
            child,
            netuid,
        ...
        JungochainModule::set_min_delegate_take(0);
        assert_ok!(JungochainModule::do_become_delegate(
            RuntimeOrigin::signed(coldkey),
            child,
            0
        ));
        ...
        pallet_jungochain::StakeDeltaSinceLastEmissionDrain::<Test>::set(parent, coldkey, -1);
        pallet_jungochain::StakeDeltaSinceLastEmissionDrain::<Test>::set(child, nominator, -1);

        // Setup YUMA so that it creates emissions:
        //   Parent and child both set weights
        //   Parent and child register on root and
        //   Set root weights
        pallet_jungochain::Weights::<Test>::insert(netuid, 0, vec![(0, 0xFFFF), (1, 0xFFFF)]);
        pallet_jungochain::Weights::<Test>::insert(netuid, 1, vec![(0, 0xFFFF), (1, 0xFFFF)]);
        assert_ok!(JungochainModule::do_root_register(
            RuntimeOrigin::signed(coldkey),
            parent,
        ));
        assert_ok!(JungochainModule::do_root_register(
        ...
        pallet_jungochain::Weights::<Test>::insert(root_id, 0, vec![(0, 0xFFFF), (1, 0xFFFF)]);
        pallet_jungochain::Weights::<Test>::insert(root_id, 1, vec![(0, 0xFFFF), (1, 0xFFFF)]);

        // Run run_coinbase until PendingHotkeyEmission are populated
        while pallet_jungochain::PendingdHotkeyEmission::<Test>::get(child) == 0 {
            step_block(1);
        }

        // Prevent further subnet epochs
        pallet_jungochain::Tempo::<Test>::set(netuid, u16::MAX);
        pallet_jungochain::Tempo::<Test>::set(root_id, u16::MAX);
        ...
        let child_emission = pallet_jungochain::Stake::<Test>::get(child, coldkey);
        let parent_emission = pallet_jungochain::Stake::<Test>::get(parent, coldkey) - stake;
        let nominator_emission = pallet_jungochain::Stake::<Test>::get(child, nominator) - stake;
...
fn test_childkey_take_drain_validator_take() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_max_delegate_take(max_take);
        assert_ok!(JungochainModule::do_become_delegate(
            RuntimeOrigin::signed(coldkey),
            child,
            max_take
...
fn test_set_children_rate_limit_fail_then_succeed() {
    new_test_ext(1).execute_with(|| {
        ...
        // Check rate limit
        let limit = JungochainModule::get_rate_limit(&TransactionType::SetChildren, netuid);

        // Step that many blocks
        step_block(limit as u16);

        // Verify rate limit passes
        assert!(JungochainModule::passes_rate_limit_on_subnet(
