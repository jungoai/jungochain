...
fn test_initialise_ti() {
    ...
    new_test_ext(1).execute_with(|| {
        pallet_jungochain::SubnetLocked::<Test>::insert(1, 100);
        pallet_jungochain::SubnetLocked::<Test>::insert(2, 5);
        pallet_balances::TotalIssuance::<Test>::put(1000);
        pallet_jungochain::TotalStake::<Test>::put(25);

        // Ensure values are NOT initialized prior to running migration
        assert!(pallet_jungochain::TotalIssuance::<Test>::get() == 0);

        pallet_jungochain::migrations::migrate_init_total_issuance::initialise_total_issuance::Migration::<Test>::on_runtime_upgrade();

        // Ensure values were initialized correctly
        assert!(
            pallet_jungochain::TotalIssuance::<Test>::get()
...
fn test_migration_fix_total_stake_maps() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&ck1, &hk1, 100);
        total_stake_amount += 100;

        JungochainModule::increase_stake_on_coldkey_hotkey_account(&ck2, &hk1, 10_101);
        total_stake_amount += 10_101;

        JungochainModule::increase_stake_on_coldkey_hotkey_account(&ck3, &hk2, 100_000_000);
        total_stake_amount += 100_000_000;

        JungochainModule::increase_stake_on_coldkey_hotkey_account(&ck1, &hk2, 1_123_000_000);
        total_stake_amount += 1_123_000_000;

        // Check that the total stake is correct
        assert_eq!(JungochainModule::get_total_stake(), total_stake_amount);

        // Check that the total coldkey stake is correct
        assert_eq!(
            JungochainModule::get_total_stake_for_coldkey(&ck1),
            100 + 1_123_000_000
        );
        assert_eq!(JungochainModule::get_total_stake_for_coldkey(&ck2), 10_101);
        assert_eq!(
            JungochainModule::get_total_stake_for_coldkey(&ck3),
            100_000_000
        );

        // Check that the total hotkey stake is correct
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hk1),
            100 + 10_101
        );
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hk2),
            100_000_000 + 1_123_000_000
        );

        // Mess up the total coldkey stake
        pallet_jungochain::TotalColdkeyStake::<Test>::insert(ck1, 0);
        // Verify that the total coldkey stake is now 0 for ck1
        assert_eq!(JungochainModule::get_total_stake_for_coldkey(&ck1), 0);

        // Mess up the total stake
        pallet_jungochain::TotalStake::<Test>::put(123_456_789);
        // Verify that the total stake is now wrong
        assert_ne!(JungochainModule::get_total_stake(), total_stake_amount);

        // Run the migration to fix the total stake maps
        pallet_jungochain::migrations::migrate_to_v2_fixed_total_stake::migrate_to_v2_fixed_total_stake::<Test>();
        ...
        assert_eq!(pallet_jungochain::Stake::<Test>::iter().count(), 4); // 4 entries total
        assert_eq!(
            pallet_jungochain::Stake::<Test>::iter_key_prefix(hk1).count(),
            2
        ); // 2 stake entries for hk1
        assert_eq!(
            pallet_jungochain::Stake::<Test>::iter_key_prefix(hk2).count(),
            2
...
// cargo test --package pallet-jungochain --test migration test_migrate_total_issuance
fn test_migrate_total_issuance() {
    new_test_ext(1).execute_with(|| {
        ...
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(test);
        assert_eq!(JungochainModule::get_total_issuance(), 0);

        JungochainModule::add_balance_to_coldkey_account(&U256::from(1), 10000);
        assert_eq!(JungochainModule::get_total_issuance(), 0);
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(test);
        assert_eq!(JungochainModule::get_total_issuance(), 10000);
        ...
        assert_eq!(JungochainModule::get_total_issuance(), 10000);
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(test);
        assert_eq!(JungochainModule::get_total_issuance(), 10000 + 30000);
    })
}

#[test]
// To run this test with cargo, use the following command:
// cargo test --package pallet-jungochain --test migration test_total_issuance_global
fn test_total_issuance_global() {
    new_test_ext(0).execute_with(|| {
        ...
        let lockcost: u64 = JungochainModule::get_network_lock_cost();
        JungochainModule::add_balance_to_coldkey_account(&owner, lockcost); // Add a balance of 20000 to the coldkey account.
        assert_eq!(JungochainModule::get_total_issuance(), 0); // initial is zero.
        assert_ok!(JungochainModule::register_network(
            <<Test as Config>::RuntimeOrigin>::signed(owner),
        ));
        JungochainModule::set_max_allowed_uids(netuid, 1); // Set the maximum allowed unique identifiers for the network to 1.
        assert_eq!(JungochainModule::get_total_issuance(), 0); // initial is zero.
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(true); // Pick up lock.
        assert_eq!(JungochainModule::get_total_issuance(), lockcost); // Verify the total issuance is updated to 20000 after migration.
        ...
        assert_eq!(JungochainModule::get_total_issuance(), lockcost); // Ensure the total issuance starts at 0 before the migration.
        JungochainModule::add_balance_to_coldkey_account(&coldkey, account_balance); // Add a balance of 20000 to the coldkey account.
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(true); // Execute the migration to update total issuance.
        assert_eq!(
        ...
        JungochainModule::set_burn(netuid, burn_cost); // Set the burn amount to 10000 for the network.
        assert_eq!(
            JungochainModule::get_total_issuance(),
            account_balance + lockcost
        ...
        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 1); // Ensure the subnetwork count increases to 1 after burning
        assert_eq!(
            JungochainModule::get_total_issuance(),
        ...
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &hotkey, new_stake); // Stake an additional 10000 to the coldkey-hotkey account. This is i
        assert_eq!(
            JungochainModule::get_total_issuance(),
            account_balance + lockcost - burn_cost
        ); // Same
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(true); // Fix issuance
        assert_eq!(
            JungochainModule::get_total_issuance(),
        ...
        JungochainModule::set_tempo(netuid, 1);
        JungochainModule::set_emission_values(&[netuid], vec![emission]).unwrap(); // Set the emission value for the network to 1_000_000_000.
        assert_eq!(
            JungochainModule::get_subnet_emission_value(netuid),
            emission
        ); // Verify the emission value is set correctly for the network.
        assert_eq!(
            JungochainModule::get_total_issuance(),
            account_balance + lockcost - burn_cost + new_stake
        );
        ...
        pallet_jungochain::migrations::migrate_total_issuance::migrate_total_issuance::<Test>(true); // Test migration does not change amount.
        assert_eq!(
            JungochainModule::get_total_issuance(),
            account_balance + lockcost - burn_cost + new_stake + emission
        ); // Verify the total issuance reflects the staked amount and emission value that has been put through the epoch.
...
fn test_migration_transfer_nets_to_foundation() {
    new_test_ext(1).execute_with(|| {
        ...
        log::info!("{:?}", JungochainModule::get_subnet_owner(1));
        //assert_eq!(JungochainModule::<T>::get_subnet_owner(1), );

        // Run the migration to transfer ownership
        let hex =
            hex_literal::hex!["feabaafee293d3b76dae304e2f9d885f77d2b17adab9e17e921b321eccd61c77"];
        pallet_jungochain::migrations::migrate_transfer_ownership_to_foundation::migrate_transfer_ownership_to_foundation::<Test>(hex);

        log::info!("new owner: {:?}", JungochainModule::get_subnet_owner(1));
    })
}
...
fn test_migration_delete_subnet_3() {
    new_test_ext(1).execute_with(|| {
        // Create subnet 3
        add_network(3, 1, 0);
        assert!(JungochainModule::if_subnet_exist(3));

        // Run the migration to transfer ownership
        pallet_jungochain::migrations::migrate_delete_subnet_3::migrate_delete_subnet_3::<Test>();
...
fn test_migration_delete_subnet_21() {
    new_test_ext(1).execute_with(|| {
        // Create subnet 21
        add_network(21, 1, 0);
        assert!(JungochainModule::if_subnet_exist(21));

        // Run the migration to transfer ownership
        pallet_jungochain::migrations::migrate_delete_subnet_21::migrate_delete_subnet_21::<Test>();
...
fn test_migrate_fix_total_coldkey_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        pallet_jungochain::migrations::migrate_fix_total_coldkey_stake::do_migrate_fix_total_coldkey_stake::<Test>();
...
fn run_migration_and_check(migration_name: &'static str) -> frame_support::weights::Weight {
    ...
    let weight: frame_support::weights::Weight = pallet_jungochain::migrations::migrate_fix_total_coldkey_stake::migrate_fix_total_coldkey_stake::<Test>();
...
fn test_migrate_commit_reveal_2() {
    new_test_ext(1).execute_with(|| {
        ...
        let pallet_prefix = twox_128("JungochainModule".as_bytes());
        ...
        // ------------------------------
        let weight =
            pallet_jungochain::migrations::migrate_commit_reveal_v2::migrate_commit_reveal_2::<Test>(
            );
...
fn run_pending_emissions_migration_and_check(
    ...
    let weight: frame_support::weights::Weight =
		pallet_jungochain::migrations::migrate_fix_pending_emission::migration::Migration::<Test>::on_runtime_upgrade();
...
// SKIP_WASM_BUILD=1 RUST_LOG=info cargo test --package pallet-jungochain --test migration -- test_migrate_fix_pending_emissions --exact --nocapture
#[test]
fn test_migrate_fix_pending_emissions() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_total_issuance(null_stake_total);
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(null_account, datura_old_hk_account),
            0
        );
        assert_eq!(
