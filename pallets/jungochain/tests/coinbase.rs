...
use pallet_jungochain::TargetStakesPerInterval;
...
fn test_hotkey_hashing() {
    new_test_ext(1).execute_with(|| {
        for i in 0..10000 {
            JungochainModule::hash_hotkey_to_u64(&U256::from(i));
        }
    });
}
...
fn test_hotkey_drain_time() {
    new_test_ext(1).execute_with(|| {
        // Block 0
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(0), 0, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(1), 0, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(2), 0, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(3), 0, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(4), 0, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(5), 0, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(6), 0, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(7), 0, 1));

        // Block 1
        assert!(JungochainModule::should_drain_hotkey(&U256::from(0), 1, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(1), 1, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(2), 1, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(3), 1, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(4), 1, 1));
        assert!(!JungochainModule::should_drain_hotkey(&U256::from(5), 1, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(6), 1, 1));
        assert!(JungochainModule::should_drain_hotkey(&U256::from(7), 1, 1));
    });
}
...
fn test_coinbase_basic() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_emission_values(&[netuid], vec![1]).unwrap();
        assert_eq!(JungochainModule::get_subnet_emission_value(netuid), 1);

        // Hotkey has no pending emission
        assert_eq!(JungochainModule::get_pending_hotkey_emission(&hotkey), 0);

        // Hotkey has same stake
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey), 1000);

        // Subnet has no pending emission.
        assert_eq!(JungochainModule::get_pending_emission(netuid), 0);
        ...
        assert_eq!(JungochainModule::get_pending_emission(netuid), 1);
        ...
        JungochainModule::set_hotkey_emission_tempo(2);
...
fn test_set_and_get_hotkey_emission_tempo() {
    new_test_ext(1).execute_with(|| {
        ...
        let default_tempo = JungochainModule::get_hotkey_emission_tempo();
        assert_eq!(default_tempo, 0); // default is 0 in mock.rs

        // Set a new hotkey emission tempo
        let new_tempo = 5;
        JungochainModule::set_hotkey_emission_tempo(new_tempo);

        // Get the updated hotkey emission tempo
        let updated_tempo = JungochainModule::get_hotkey_emission_tempo();
...
fn test_get_nonviable_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(
            JungochainModule::get_nonviable_stake(&delegate_hotkey, &delegate_coldkey),
            0
        );
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_overflow -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_overflow() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey, 1000);
        JungochainModule::add_balance_to_coldkey_account(&nominator1, 1500);
        JungochainModule::add_balance_to_coldkey_account(&nominator2, 1500);
        ...
        log::debug!(
            ...
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey, &hotkey),
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator1, &hotkey),
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator2, &hotkey)
        );
        log::debug!("Stakes added for nominators");

        // 5. Set emission and verify initial states
        let to_emit = 20_000e9 as u64;
        JungochainModule::set_emission_values(&[netuid], vec![to_emit]).unwrap();
        assert_eq!(JungochainModule::get_subnet_emission_value(netuid), to_emit);
        assert_eq!(JungochainModule::get_pending_hotkey_emission(&hotkey), 0);
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hotkey),
            initial_stake * 2
        ...
        JungochainModule::set_hotkey_emission_tempo(1);
        log::debug!("Hotkey emission tempo set to 1");

        // 7. Simulate blocks and check emissions
        next_block();
        assert_eq!(JungochainModule::get_pending_emission(netuid), to_emit);
        log::debug!(
        ...
        let hotkey_stake = JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey, &hotkey);
        ...
        let total_stake = JungochainModule::get_total_stake_for_hotkey(&hotkey);
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_no_deltas -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_no_deltas() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add the stake directly to their coldkey-hotkey account
        // This bypasses the accounting in stake delta
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&nominator1, &hotkey, 100);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&nominator2, &hotkey, 100);
        ...
        // 5. Set emission and verify initial states
        JungochainModule::set_emission_values(&[netuid], vec![10]).unwrap();
        assert_eq!(JungochainModule::get_subnet_emission_value(netuid), 10);
        assert_eq!(JungochainModule::get_pending_hotkey_emission(&hotkey), 0);
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey), 200);
        assert_eq!(JungochainModule::get_pending_emission(netuid), 0);
        ...
        // 7. Simulate blocks and check emissions
        next_block();
        assert_eq!(JungochainModule::get_pending_emission(netuid), 10);
        log::debug!(
            "After first block, pending emission: {}",
            JungochainModule::get_pending_emission(netuid)
        );
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_with_positive_delta -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_with_positive_delta() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::do_add_stake(
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_with_negative_delta -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_with_negative_delta() {
    new_test_ext(1).execute_with(|| {
        ...
        // 8. Check final stakes
        let delegate_stake = JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey, &hotkey);
        let total_hotkey_stake = JungochainModule::get_total_stake_for_hotkey(&hotkey);
        let nominator1_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator1, &hotkey);
        let nominator2_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator2, &hotkey);
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_with_neutral_delta -- --nocapture
#[test]
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_with_net_positive_delta -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_with_net_positive_delta() {
    new_test_ext(1).execute_with(|| {
        ...
        let initial_nominator1_stake =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator1, &hotkey);
        let intial_total_hotkey_stake = JungochainModule::get_total_stake_for_hotkey(&hotkey);
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_coinbase_nominator_drainage_with_net_negative_delta -- --nocapture
#[test]
fn test_coinbase_nominator_drainage_with_net_negative_delta() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add the stake directly to their coldkey-hotkey account
        // This bypasses the accounting in stake delta
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&nominator1, &hotkey, 300);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&nominator2, &hotkey, 300);
        ...
        let total_stake_before = JungochainModule::get_total_stake_for_hotkey(&hotkey);
        let nominator_1_stake_before =
            JungochainModule::get_stake_for_coldkey_and_hotkey(&nominator1, &hotkey);
        // Notice that nominator1 stake is the new stake, including the removed stake
        assert_eq!(
...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-jungochain --test coinbase test_emission_with_registration_disabled_subnet -- --nocapture
...
fn test_emission_with_registration_disabled_subnet() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_network_registration_allowed(netuid, false); // Disable registration
        ...
        assert_eq!(
            JungochainModule::get_pending_hotkey_emission(&hotkey),
            0,
