...
fn test_add_stake_dispatch_info_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::add_stake {
            hotkey,
            amount_staked,
        });
        assert_eq!(
...
fn test_add_stake_ok_no_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(JungochainModule::get_total_stake(), 0);
        ...
        assert_eq!(JungochainModule::get_total_stake(), 9999);
    });
}
...
fn test_dividends_with_run_to_block() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_max_registrations_per_block(netuid, 3);
        JungochainModule::set_max_allowed_uids(1, 5);
        ...
        JungochainModule::increase_stake_on_hotkey_account(&neuron_src_hotkey_id, initial_stake);

        // Check if the initial stake has arrived
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&neuron_src_hotkey_id),
            initial_stake
        );

        // Check if all three neurons are registered
        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 3);
        ...
        // Check if the stake is equal to the inital stake + transfer
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&neuron_dest_hotkey_id),
            0
        );
    });
}
...
fn test_add_stake_err_signature() {
    new_test_ext(1).execute_with(|| {
        ...
        let result = JungochainModule::add_stake(
...
fn test_add_stake_not_registered_key_pair() {
    new_test_ext(1).execute_with(|| {
        ...
        let amount = 1337;
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, 1800);
        assert_eq!(
            JungochainModule::add_stake(
                <<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id),
                hotkey_account_id,
                amount
            ),
...
fn test_add_stake_err_neuron_does_not_belong_to_coldkey() {
    new_test_ext(1).execute_with(|| {
        ...
        // Give it some $$$ in his coldkey balance
        JungochainModule::add_balance_to_coldkey_account(&other_cold_key, 100000);
...
fn test_add_stake_err_not_enough_belance() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(JungochainModule::get_coldkey_balance(&coldkey_id), 0);
        let result = JungochainModule::add_stake(
...
fn test_add_stake_total_balance_no_change() {
    ...
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, initial_balance);

        // Check we have zero staked before transfer
        let initial_stake = JungochainModule::get_total_stake_for_hotkey(&hotkey_account_id);
        ...
        // Check if stake has increased
        let new_stake = JungochainModule::get_total_stake_for_hotkey(&hotkey_account_id);
        assert_eq!(new_stake, 10000);

        // Check if free balance has decreased
        let new_free_balance = JungochainModule::get_coldkey_balance(&coldkey_account_id);
        assert_eq!(new_free_balance, 0);

        // Check if total stake has increased accordingly.
        assert_eq!(JungochainModule::get_total_stake(), 10000);
...
fn test_reset_stakes_per_interval() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_stake_interval(7);
        JungochainModule::set_stakes_this_interval_for_coldkey_hotkey(&coldkey, &hotkey, 5, 1);
        step_block(1);

        assert_eq!(
            JungochainModule::get_stakes_this_interval_for_coldkey_hotkey(&coldkey, &hotkey),
            5
        );
...
fn test_add_stake_under_limit() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_target_stakes_per_interval(max_stakes);
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, hotkey_account_id, coldkey_account_id, start_nonce);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, 60000);
        assert_ok!(JungochainModule::add_stake(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id),
            hotkey_account_id,
            1,
        ...
        let current_stakes = JungochainModule::get_stakes_this_interval_for_coldkey_hotkey(
            &coldkey_account_id,
            &hotkey_account_id,
        );
...
fn test_add_stake_rate_limit_exceeded() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_target_stakes_per_interval(max_stakes);
        JungochainModule::set_stakes_this_interval_for_coldkey_hotkey(
            &coldkey_account_id,
            &hotkey_account_id,
            max_stakes,
            block_number,
...
fn test_remove_stake_under_limit() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_target_stakes_per_interval(max_unstakes);
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, hotkey_account_id, coldkey_account_id, start_nonce);
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, 60000);
        JungochainModule::increase_stake_on_hotkey_account(&hotkey_account_id, 2);

        assert_ok!(JungochainModule::remove_stake(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id),
            hotkey_account_id,
            1,
        ));
        ...
        let current_unstakes = JungochainModule::get_stakes_this_interval_for_coldkey_hotkey(
            &coldkey_account_id,
            &hotkey_account_id,
        );
...
fn test_remove_stake_dispatch_info_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::remove_stake {
            hotkey,
            amount_unstaked,
        });
        assert_eq!(
...
fn test_remove_stake_ok_no_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_hotkey_account(&hotkey_account_id, amount);
...
fn test_remove_stake_err_signature() {
    new_test_ext(1).execute_with(|| {
        ...
        let result = JungochainModule::remove_stake(
            <<Test as Config>::RuntimeOrigin>::none(),
            hotkey_account_id,
            amount,
        );
...
fn test_remove_stake_no_enough_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey_id), 0);
...
fn test_get_coldkey_balance_no_balance() {
    new_test_ext(1).execute_with(|| {
        ...
        let result = JungochainModule::get_coldkey_balance(&coldkey_account_id);
...
fn test_get_coldkey_balance_with_balance() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, amount);
...
fn test_add_stake_to_hotkey_account_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_stake_on_hotkey_account(&hotkey_id, amount);

        // The stake that is now in the account, should equal the amount
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hotkey_id),
            amount
        );

        // The total stake should have been increased by the amount -> 0 + amount = amount
        assert_eq!(JungochainModule::get_total_stake(), amount);
    });
}
...
fn test_remove_stake_from_hotkey_account() {
    new_test_ext(1).execute_with(|| {
        ...
        // Remove stake
        JungochainModule::decrease_stake_on_hotkey_account(&hotkey_id, amount);
...
fn test_remove_stake_from_hotkey_account_registered_in_various_networks() {
    new_test_ext(1).execute_with(|| {
        ...
        //let neuron_uid = JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey_id);

        let neuron_uid = match JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey_id) {
            Ok(k) => k,
            Err(e) => panic!("Error: {:?}", e),
        };
        //let neuron_uid_ex = JungochainModule::get_uid_for_net_and_hotkey(netuid_ex, &hotkey_id);

        let neuron_uid_ex =
            match JungochainModule::get_uid_for_net_and_hotkey(netuid_ex, &hotkey_id) {
                Ok(k) => k,
                Err(e) => panic!("Error: {:?}", e),
            };
        //Add some stake that can be removed
        ...
        assert_eq!(
            JungochainModule::get_stake_for_uid_and_subnetwork(netuid_ex, neuron_uid_ex),
            amount
        );
...
fn test_increase_total_stake_ok() {
    new_test_ext(1).execute_with(|| {
        let increment = 10000;
        assert_eq!(JungochainModule::get_total_stake(), 0);
        JungochainModule::increase_total_stake(increment);
        assert_eq!(JungochainModule::get_total_stake(), increment);
    });
}
...
fn test_decrease_total_stake_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increase_total_stake(initial_total_stake);
        JungochainModule::decrease_total_stake(decrement);

        // The total stake remaining should be the difference between the initial stake and the decrement
        assert_eq!(
            JungochainModule::get_total_stake(),
...
fn test_add_balance_to_coldkey_account_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let amount = 50000;
        JungochainModule::add_balance_to_coldkey_account(&coldkey_id, amount);
        assert_eq!(JungochainModule::get_coldkey_balance(&coldkey_id), amount);
    });
}
...
fn test_remove_balance_from_coldkey_account_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, ammount);
        assert_eq!(
            JungochainModule::get_coldkey_balance(&coldkey_account_id),
            ammount
        );
        ...
        let result =
            JungochainModule::remove_balance_from_coldkey_account(&coldkey_account_id, ammount);
        assert!(result.is_ok());
    });
}
...
fn test_hotkey_belongs_to_coldkey_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        register_ok_neuron(netuid, hotkey_id, coldkey_id, start_nonce);
        assert_eq!(
            JungochainModule::get_owning_coldkey_for_hotkey(&hotkey_id),
            coldkey_id
        );
    });
}
...
fn test_can_remove_balane_from_coldkey_account_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey_id, initial_amount);
        assert!(JungochainModule::can_remove_balance_from_coldkey_account(
            &coldkey_id,
            remove_amount
        ));
    });
...
fn test_has_enough_stake_yes() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, hotkey_id, coldkey_id, start_nonce);
        JungochainModule::increase_stake_on_hotkey_account(&hotkey_id, intial_amount);
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hotkey_id),
            10000
        );
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey_id, &hotkey_id),
            10000
        );
        assert!(JungochainModule::has_enough_stake(
            &coldkey_id,
            &hotkey_id,
            5000
        ));
...
fn test_non_existent_account() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&U256::from(0), &U256::from(0)),
            10
        );
        assert_eq!(
            JungochainModule::get_total_stake_for_coldkey(&(U256::from(0))),
            10
        );
    });
}
...
fn test_delegate_stake_division_by_zero_check() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::become_delegate(
...
fn test_unstake_all_coldkeys_from_hotkey_account() {
    new_test_ext(1).execute_with(|| {
        ...
        match JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey_id) {
            Ok(_k) => (),
            Err(e) => panic!("Error: {:?}", e),
        }
        ...
        JungochainModule::unstake_all_coldkeys_from_hotkey_account(&hotkey_id);
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey0_id, &hotkey_id),
            0
        );
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey1_id, &hotkey_id),
            0
        );
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey2_id, &hotkey_id),
            0
        );
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey3_id, &hotkey_id),
            0
        );
...
fn test_faucet_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let block_number = JungochainModule::get_current_block_as_u64();
        let difficulty: U256 = U256::from(10_000_000);
        let mut nonce: u64 = 0;
        let mut work: H256 = JungochainModule::create_seal_hash(block_number, nonce, &coldkey);
        while !JungochainModule::hash_meets_difficulty(&work, difficulty) {
            nonce += 1;
            work = JungochainModule::create_seal_hash(block_number, nonce, &coldkey);
        }
        let vec_work: Vec<u8> = JungochainModule::hash_to_vec(work);

        log::info!("Faucet state: {}", cfg!(feature = "pow-faucet"));

        #[cfg(feature = "pow-faucet")]
        assert_ok!(JungochainModule::do_faucet(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey),
            block_number,
            nonce,
            vec_work
...
/// Run this test using: cargo test --package pallet-jungochain --test staking test_clear_small_nominations
#[test]
fn test_clear_small_nominations() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_target_stakes_per_interval(10);
        ...
        assert_ok!(JungochainModule::do_become_delegate(
            ...
            hot1,
            JungochainModule::get_min_delegate_take()
        ));
        assert_eq!(
            JungochainModule::get_owning_coldkey_for_hotkey(&hot1),
            cold1
        );
        ...
        assert_eq!(
            JungochainModule::get_owning_coldkey_for_hotkey(&hot2),
            cold2
        );

        // Add stake cold1 --> hot1 (non delegation.)
        JungochainModule::add_balance_to_coldkey_account(&cold1, 5);
        assert_ok!(JungochainModule::add_stake(
            <<Test as Config>::RuntimeOrigin>::signed(cold1),
            hot1,
            1
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&cold1, &hot1),
            1
        );
        assert_eq!(Balances::free_balance(cold1), 4);

        // Add stake cold2 --> hot1 (is delegation.)
        JungochainModule::add_balance_to_coldkey_account(&cold2, 5);
        assert_ok!(JungochainModule::add_stake(
            <<Test as Config>::RuntimeOrigin>::signed(cold2),
            hot1,
            1
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&cold2, &hot1),
            1
        );
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&cold1, &hot2),
            1
        );
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&cold2, &hot2),
            1
        );
        assert_eq!(Balances::free_balance(cold2), 8);

        // Run clear all small nominations when min stake is zero (noop)
        JungochainModule::set_nominator_min_required_stake(0);
        JungochainModule::clear_small_nominations();
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&cold1, &hot1),
            1
        );
        ...
        JungochainModule::set_nominator_min_required_stake(10);
...
fn test_add_stake_below_minimum_threshold() {
    new_test_ext(0).execute_with(|| {
        ...
        // Add balances.
        JungochainModule::add_balance_to_coldkey_account(&coldkey1, 100_000);
        JungochainModule::add_balance_to_coldkey_account(&coldkey2, 100_000);
        JungochainModule::set_nominator_min_required_stake(minimum_threshold);
        JungochainModule::set_target_stakes_per_interval(10);
        ...
        assert_noop!(
            ...
            pallet_jungochain::Error::<Test>::NomStakeBelowMinimumThreshold
        );
    });
}
...
fn test_remove_stake_below_minimum_threshold() {
    new_test_ext(0).execute_with(|| {
        ...
        // Add balances.
        JungochainModule::add_balance_to_coldkey_account(&coldkey1, initial_balance);
        JungochainModule::add_balance_to_coldkey_account(&coldkey2, initial_balance);
        JungochainModule::set_nominator_min_required_stake(minimum_threshold);
        JungochainModule::set_target_stakes_per_interval(10);
        ...
        let total_hotkey_stake_before = JungochainModule::get_total_stake_for_hotkey(&hotkey1);
        let bal_before = Balances::free_balance(coldkey2);
        let staked_before = JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey2, &hotkey1);
        let total_network_stake_before = JungochainModule::get_total_stake();
        let total_issuance_before = JungochainModule::get_total_issuance();
        ...
        // Has no stake now
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&coldkey2, &hotkey1),
            0
        );
        ...
        assert_eq!(
            JungochainModule::get_total_issuance(),
...
fn test_delegate_take_can_be_decreased() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add balance
        JungochainModule::add_balance_to_coldkey_account(&coldkey0, 100000);
...
fn test_can_set_min_take_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::do_decrease_take(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey0),
            hotkey0,
            JungochainModule::get_min_delegate_take()
        ));
...
fn test_delegate_take_can_be_increased() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::do_increase_take(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey0),
            hotkey0,
            u16::MAX / 8
        ));
        assert_eq!(JungochainModule::get_hotkey_take(&hotkey0), u16::MAX / 8);
    });
}
...
fn test_delegate_take_can_not_be_decreased_with_increase_take() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(
            JungochainModule::do_increase_take(
                <<Test as Config>::RuntimeOrigin>::signed(coldkey0),
                hotkey0,
                u16::MAX / 20
            ),
...
fn test_delegate_take_can_not_be_set_beyond_limit() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 0, 0);
        register_ok_neuron(netuid, hotkey0, coldkey0, 124124);
        let before = JungochainModule::get_hotkey_take(&hotkey0);
        ...
        if InitialDefaultDelegateTake::get() != u16::MAX {
            assert_eq!(
                JungochainModule::do_become_delegate(
                    <<Test as Config>::RuntimeOrigin>::signed(coldkey0),
                    hotkey0,
        ...
        assert_eq!(JungochainModule::get_hotkey_take(&hotkey0), before);
    });
}
...
fn test_get_total_delegated_stake_after_unstaking() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&delegator, initial_stake);
        ...
        assert_eq!(
            JungochainModule::get_total_delegated_stake(&delegate_coldkey),
...
fn test_get_total_delegated_stake_no_delegations() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(JungochainModule::get_total_delegated_stake(&delegate), 0);
    });
}
...
fn test_get_total_delegated_stake_single_delegator() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&delegator, stake_amount);
        assert_ok!(JungochainModule::add_stake(
            RuntimeOrigin::signed(delegator),
            delegate_hotkey,
            stake_amount
        ...
        println!("Existential deposit: {}", existential_deposit);
        println!("Total stake for hotkey: {}", JungochainModule::get_total_stake_for_hotkey(&delegate_hotkey));
        println!("Delegated stake for coldkey: {}", JungochainModule::get_total_delegated_stake(&delegate_coldkey));

        // Calculate expected delegated stake
        let expected_delegated_stake = stake_amount - existential_deposit;
        let actual_delegated_stake = JungochainModule::get_total_delegated_stake(&delegate_coldkey);
...
fn test_get_total_delegated_stake_multiple_delegators() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&delegator1, stake1);
        assert_ok!(JungochainModule::add_stake(
        ...
        JungochainModule::add_balance_to_coldkey_account(&delegator2, stake2);
        assert_ok!(JungochainModule::add_stake(
        ...
        let actual_total_delegated = JungochainModule::get_total_delegated_stake(&delegate_coldkey);
...
fn test_get_total_delegated_stake_exclude_owner_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        // Add owner stake
        JungochainModule::add_balance_to_coldkey_account(&delegate_coldkey, owner_stake);
        assert_ok!(JungochainModule::add_stake(
            RuntimeOrigin::signed(delegate_coldkey),
            delegate_hotkey,
            owner_stake
        ...
        // Add delegator stake
        JungochainModule::add_balance_to_coldkey_account(&delegator, delegator_stake);
        assert_ok!(JungochainModule::add_stake(
            RuntimeOrigin::signed(delegator),
            delegate_hotkey,
            delegator_stake
        ...
        println!(
            ...
            JungochainModule::get_total_stake_for_hotkey(&delegate_hotkey)
        );
        println!(
...
fn test_stake_delta_tracks_adds_and_removes() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&delegator, delegator_added_stake);
        assert_ok!(JungochainModule::add_stake(
            RuntimeOrigin::signed(delegator),
            delegate_hotkey,
...
fn test_mining_emission_drain() {
    new_test_ext(1).execute_with(|| {
        ...
        // All stake is active
        pallet_jungochain::ActivityCutoff::<Test>::set(netuid, u16::MAX);
        // There's only one validator
        pallet_jungochain::MaxAllowedUids::<Test>::set(netuid, 2);
        JungochainModule::set_max_allowed_validators(netuid, 1);
        ...
        pallet_jungochain::StakeDeltaSinceLastEmissionDrain::<Test>::set(validator, coldkey, -1);
        pallet_jungochain::StakeDeltaSinceLastEmissionDrain::<Test>::set(miner, nominator, -1);
        ...
        pallet_jungochain::Weights::<Test>::insert(netuid, 0, vec![(1, 0xFFFF)]);
        assert_ok!(JungochainModule::do_root_register(
            RuntimeOrigin::signed(coldkey),
            validator,
        ));
        pallet_jungochain::Weights::<Test>::insert(root_id, 0, vec![(0, 0xFFFF), (1, 0xFFFF)]);
        pallet_jungochain::BlockAtRegistration::<Test>::set(netuid, 0, 1);
        pallet_jungochain::LastUpdate::<Test>::set(netuid, vec![2, 2]);
        pallet_jungochain::Kappa::<Test>::set(netuid, u16::MAX / 5);

        // Run run_coinbase until root epoch is run
        while pallet_jungochain::PendingEmission::<Test>::get(netuid) == 0 {
            step_block(1);
        }
        ...
        while pallet_jungochain::PendingdHotkeyEmission::<Test>::get(miner) == 0 {
            step_block(1);
        }
        ...
        let miner_emission = pallet_jungochain::Stake::<Test>::get(miner, coldkey) - miner_stake;
        let validator_emission = pallet_jungochain::Stake::<Test>::get(validator, coldkey) - stake;
        let nominator_emission = pallet_jungochain::Stake::<Test>::get(miner, nominator) - stake;
...
fn test_mining_emission_drain_with_validation() {
    new_test_ext(1).execute_with(|| {
        ...
        // Make all stakes viable
        pallet_jungochain::StakeDeltaSinceLastEmissionDrain::<Test>::set(
            validator_miner1,
            coldkey,
            -1,
        );
        ...
        pallet_jungochain::Weights::<Test>::insert(root_id, 0, vec![(0, 0xFFFF), (1, 0xFFFF)]);
        pallet_jungochain::BlockAtRegistration::<Test>::set(netuid, 0, 1);
        pallet_jungochain::BlockAtRegistration::<Test>::set(netuid, 1, 1);
        pallet_jungochain::LastUpdate::<Test>::set(netuid, vec![2, 2]);
        pallet_jungochain::Kappa::<Test>::set(netuid, u16::MAX / 5);
        ...
        // Run run_coinbase until PendingHotkeyEmission are populated
        while pallet_jungochain::PendingdHotkeyEmission::<Test>::get(validator_miner1) == 0 {
            step_block(1);
        }
        ...
        let validator_miner_emission1 =
            pallet_jungochain::Stake::<Test>::get(validator_miner1, coldkey) - stake;
        let validator_miner_emission2 =
            pallet_jungochain::Stake::<Test>::get(validator_miner2, coldkey) - half_stake;
        let nominator_emission =
            pallet_jungochain::Stake::<Test>::get(validator_miner2, nominator) - half_stake;
        let total_emission =
...
fn test_mining_emission_drain_validator_valiminer_miner() {
    new_test_ext(1).execute_with(|| {
        ...
        pallet_jungochain::MaxAllowedUids::<Test>::set(netuid, 3);
        JungochainModule::set_max_allowed_validators(netuid, 2);
        ...
        //   Last weight update is after block at registration
        pallet_jungochain::Weights::<Test>::insert(netuid, 0, vec![(1, 0xFFFF)]);
        pallet_jungochain::Weights::<Test>::insert(netuid, 1, vec![(2, 0xFFFF)]);
        assert_ok!(JungochainModule::do_root_register(
            RuntimeOrigin::signed(coldkey),
            validator,
        ));
        ...
        pallet_jungochain::BlockAtRegistration::<Test>::set(netuid, 0, 1);
        pallet_jungochain::BlockAtRegistration::<Test>::set(netuid, 1, 1);
        pallet_jungochain::LastUpdate::<Test>::set(netuid, vec![2, 2, 2]);
        pallet_jungochain::Kappa::<Test>::set(netuid, u16::MAX / 5);
        ...
        // Run run_coinbase until PendingHotkeyEmission are populated
        while pallet_jungochain::PendingdHotkeyEmission::<Test>::get(validator) == 0 {
            step_block(1);
        }
        ...
        let valiminer_emission =
            pallet_jungochain::Stake::<Test>::get(validator_miner, coldkey) - stake;
        let miner_emission = pallet_jungochain::Stake::<Test>::get(miner, coldkey);
