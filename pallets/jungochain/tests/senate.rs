...
use pallet_jungochain::migrations;
...
fn test_senate_join_works() {
    new_test_ext().execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&staker_coldkey, 100_000);
        ...
        assert_eq!(
            JungochainModule::get_stake_for_coldkey_and_hotkey(&staker_coldkey, &hotkey_account_id),
            99_999
        );
        assert_eq!(
...
fn test_senate_vote_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(JungochainModule::do_vote_root(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id),
            &hotkey_account_id,
            hash,
            0,
...
fn test_senate_leave_vote_removal() {
    new_test_ext().execute_with(|| {
        ...
        JungochainModule::set_max_registrations_per_block(other_netuid, 1000);
        JungochainModule::set_target_registrations_per_interval(other_netuid, 1000);
        ...
        for i in 0..200 {
            ...
            // Add balance
            JungochainModule::add_balance_to_coldkey_account(&cold, 100_000_000 + (i as u64)); // lots ot stake
                                                                                               // Register
            assert_ok!(JungochainModule::burned_register(
        ...
        assert!(
            JungochainModule::get_uid_for_net_and_hotkey(root_netuid, &hotkey_account_id).is_err()
        );
...
fn test_senate_not_leave_when_stake_removed() {
    new_test_ext().execute_with(|| {
        ...
        JungochainModule::set_target_stakes_per_interval(2);
        ...
        JungochainModule::add_balance_to_coldkey_account(&staker_coldkey, stake_amount);
...
fn test_senate_join_current_delegate() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(JungochainModule::adjust_senate(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id),
            hotkey_account_id
        ));
        ...
        // Check the events
        assert!(
            System::events().contains(&record(RuntimeEvent::JungochainModule(
                JungochainEvent::SenateAdjusted {
                    old_member: None,
...
fn test_adjust_senate_events() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let burn_cost = 1000;
        let coldkey_account_id = U256::from(667);
        let root_netuid = JungochainModule::get_root_netuid();
        ...
        add_network(netuid, tempo, 0);
        // Give some coldkey balance
        JungochainModule::add_balance_to_coldkey_account(&coldkey_account_id, balance_to_add);

        // Allow all registrations in netuid in same block. Same for root network.
        JungochainModule::set_max_registrations_per_block(netuid, max_senate_size + 1);
        JungochainModule::set_target_registrations_per_interval(netuid, max_senate_size + 1);
        JungochainModule::set_max_registrations_per_block(root_netuid, max_senate_size + 1);
        JungochainModule::set_target_registrations_per_interval(root_netuid, max_senate_size + 1);
        ...
        for i in 0..(max_senate_size - 1) {
            ...
            assert_eq!(
                JungochainModule::get_owning_coldkey_for_hotkey(&new_hotkey_account_id),
                coldkey_account_id
            );
        ...
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&replacement_hotkey_account_id),
            1
        );
