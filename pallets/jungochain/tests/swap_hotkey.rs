...
fn test_swap_owner() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::perform_hotkey_swap(
            &old_hotkey,
            &new_hotkey,
            &coldkey,
            &mut weight
...
fn test_swap_last_tx_block_delegate_take() {
    new_test_ext(1).execute_with(|| {
        ...
        pallet_jungochain::LastTxBlockDelegateTake::<Test>::insert(old_hotkey, 1000);
        assert_ok!(JungochainModule::perform_hotkey_swap(
            &old_hotkey,
            &new_hotkey,
            &coldkey,
...
fn test_swap_hotkey_tx_rate_limit_exceeded() {
    new_test_ext(1).execute_with(|| {
        ...
        let current_tx_rate_limit = JungochainModule::get_tx_rate_limit();
        log::info!("current_tx_rate_limit: {:?}", current_tx_rate_limit);

        // Set the transaction rate limit
        JungochainModule::set_tx_rate_limit(tx_rate_limit);
        // assert the rate limit is set to 1000 blocks
        assert_eq!(JungochainModule::get_tx_rate_limit(), tx_rate_limit);

        // Setup initial state
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, old_hotkey, coldkey, 0);
        JungochainModule::add_balance_to_coldkey_account(&coldkey, swap_cost);

        // Perform the first swap
        assert_ok!(JungochainModule::do_swap_hotkey(
        ...
        assert_err!(
            JungochainModule::do_swap_hotkey(
                <<Test as Config>::RuntimeOrigin>::signed(coldkey),
                &new_hotkey_1,
                &new_hotkey_2
            ),
...
fn test_do_swap_hotkey_err_not_owner() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, old_hotkey, coldkey, 0);
        JungochainModule::add_balance_to_coldkey_account(&not_owner_coldkey, swap_cost);
...
fn test_swap_owner_success() {
    new_test_ext(1).execute_with(|| {
        ...
        // Perform the swap
        JungochainModule::perform_hotkey_swap(&old_hotkey, &new_hotkey, &coldkey, &mut weight);
...
fn test_swap_hotkey_error_cases() {
    new_test_ext(1).execute_with(|| {
        ...
        let initial_balance = JungochainModule::get_key_swap_cost() + 1000;
        JungochainModule::add_balance_to_coldkey_account(&coldkey, initial_balance);
...
fn test_swap_parent_hotkey_childkey_maps() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::create_account_if_non_existent(&coldkey, &parent_old);
...
fn test_swap_child_hotkey_childkey_maps() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 0, 0);
        JungochainModule::create_account_if_non_existent(&coldkey, &child_old);
        JungochainModule::create_account_if_non_existent(&coldkey, &parent);
