...
use pallet_jungochain::{Call, ColdkeySwapScheduleDuration, Error};
use sp_core::H256;
use sp_core::U256;
...
fn test_swap_total_hotkey_coldkey_stakes_this_interval() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::perform_swap_coldkey(
            &old_coldkey,
            &new_coldkey,
            &mut weight
        ));
...
fn test_transfer_remaining_balance() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, balance);
        ...
        assert_eq!(JungochainModule::get_coldkey_balance(&old_coldkey), 0);
        assert_eq!(JungochainModule::get_coldkey_balance(&new_coldkey), balance);
    });
}
...
fn test_swap_concurrent_modifications() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&new_coldkey, additional_stake);
...
fn test_do_swap_coldkey_success() {
    new_test_ext(1).execute_with(|| {
        ...
        let swap_cost = JungochainModule::get_key_swap_cost();
        ...
        log::info!(
            ...
            JungochainModule::get_total_stake_for_coldkey(&old_coldkey)
        );
        log::info!(
            "Initial new coldkey stake: {}",
            JungochainModule::get_total_stake_for_coldkey(&new_coldkey)
        );
        ...
        let total_stake_before_swap = JungochainModule::get_total_stake();

        // Perform the swap
        assert_ok!(JungochainModule::do_swap_coldkey(
            // <<Test as Config>::RuntimeOrigin>::signed(old_coldkey),
            &old_coldkey,
            &new_coldkey
        ));
        ...
        assert_eq!(
            JungochainModule::get_coldkey_balance(&new_coldkey),
...
fn test_swap_stake_for_coldkey() {
    new_test_ext(1).execute_with(|| {
        ...
        let initial_total_issuance = JungochainModule::get_total_issuance();
        let initial_total_stake = JungochainModule::get_total_stake();

        // Perform the swap
        JungochainModule::perform_swap_coldkey(&old_coldkey, &new_coldkey, &mut weight);
        ...
        assert_eq!(
            JungochainModule::get_owned_hotkeys(&new_coldkey),
            vec![hotkey1, hotkey2]
        );
        assert_eq!(JungochainModule::get_owned_hotkeys(&old_coldkey), vec![]);
...
fn test_swap_subnet_owner_for_coldkey() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increment_total_subnets(netuid1);
        JungochainModule::increment_total_subnets(netuid2);
...
fn test_do_swap_coldkey_with_subnet_ownership() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::increment_total_subnets(netuid);

        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, stake_amount + swap_cost);
        SubnetOwner::<Test>::insert(netuid, old_coldkey);
...
fn test_coldkey_swap_total() {
    new_test_ext(1).execute_with(|| {
        ...
        let netuid3 = 3u16;
        JungochainModule::add_balance_to_coldkey_account(&coldkey, 1000);
        JungochainModule::add_balance_to_coldkey_account(&delegate1, 1000);
        JungochainModule::add_balance_to_coldkey_account(&delegate2, 1000);
        JungochainModule::add_balance_to_coldkey_account(&delegate3, 1000);
        JungochainModule::add_balance_to_coldkey_account(&nominator1, 1000);
        JungochainModule::add_balance_to_coldkey_account(&nominator2, 1000);
        JungochainModule::add_balance_to_coldkey_account(&nominator3, 1000);
        ...
        assert_eq!(
            JungochainModule::get_owned_hotkeys(&coldkey),
            vec![hotkey1, hotkey2, hotkey3]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&coldkey),
            vec![hotkey1, hotkey2, hotkey3, delegate1, delegate2, delegate3]
        );
        assert_eq!(JungochainModule::get_total_stake_for_coldkey(&coldkey), 600);
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey1), 300);
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey2), 300);
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&hotkey3), 300);
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&delegate1),
            300
        );
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&delegate2),
            300
        );
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&delegate3),
            300
        );

        assert_eq!(
            JungochainModule::get_owned_hotkeys(&delegate1),
            vec![delegate1]
        );
        assert_eq!(
            JungochainModule::get_owned_hotkeys(&delegate2),
            vec![delegate2]
        );
        assert_eq!(
            JungochainModule::get_owned_hotkeys(&delegate3),
            vec![delegate3]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&delegate1),
            vec![delegate1, hotkey1]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&delegate2),
            vec![delegate2, hotkey2]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&delegate3),
            vec![delegate3, hotkey3]
        );

        assert_eq!(JungochainModule::get_owned_hotkeys(&nominator1), vec![]);
        assert_eq!(JungochainModule::get_owned_hotkeys(&nominator2), vec![]);
        assert_eq!(JungochainModule::get_owned_hotkeys(&nominator3), vec![]);

        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&nominator1),
            vec![hotkey1, delegate1]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&nominator2),
            vec![hotkey2, delegate2]
        );
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&nominator3),
        ...
        assert_eq!(
            JungochainModule::get_all_staked_hotkeys(&new_coldkey),
            vec![hotkey1, hotkey2, hotkey3, delegate1, delegate2, delegate3]
        );
        assert_eq!(
            JungochainModule::get_total_stake_for_coldkey(&new_coldkey),
...
fn test_swap_senate_member() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::swap_senate_member(
            &old_hotkey,
            &new_hotkey,
            &mut weight
        ));
...
fn test_coldkey_delegations() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(JungochainModule::get_total_stake_for_hotkey(&delegate), 100);
        assert_eq!(JungochainModule::get_total_stake_for_coldkey(&coldkey), 0);
        assert_eq!(
            JungochainModule::get_total_stake_for_coldkey(&new_coldkey),
            100
        );
...
fn test_schedule_swap_coldkey_success() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, 1000);
...
fn test_schedule_swap_coldkey_duplicate() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, 2000);
        ...
        assert_noop!(
            JungochainModule::schedule_swap_coldkey(
                <<Test as Config>::RuntimeOrigin>::signed(old_coldkey),
                new_coldkey
            ),
...
fn test_schedule_swap_coldkey_execution() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 13, 0);
        register_ok_neuron(netuid, hotkey, old_coldkey, 0);
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, 1000000000000000);
        assert_ok!(JungochainModule::add_stake(
            <<Test as Config>::RuntimeOrigin>::signed(old_coldkey),
            hotkey,
            stake_amount
        ...
        JungochainModule::on_initialize(execution_block);
...
fn test_coldkey_swap_delegate_identity_updated() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, 100_000_000_000);
...
fn test_coldkey_swap_stake_delta() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&old_coldkey, 100e9 as u64);
