...
use sp_core::{ConstU16, ConstU64, Get, H256, U256};
...
frame_support::construct_runtime!(
    pub enum Test
    {
        ...
        JungochainModule: pallet_jungochain::{Pallet, Call, Storage, Event<T>} = 7,
...
impl pallet_jungochain::Config for Test {
    ...
    type DefaultStakeInterval = ConstU64<99>;
    type DefaultWeightsSetRateLimit = ConstU64<9>;
}
...
pub(crate) fn step_epochs(count: u16, netuid: u16) {
    for _ in 0..count {
        let blocks_to_next_epoch = JungochainModule::blocks_until_next_epoch(
            netuid,
            JungochainModule::get_tempo(netuid),
            JungochainModule::get_current_block_as_u64(),
        );
        step_block(blocks_to_next_epoch as u16);

        assert!(JungochainModule::should_run_epoch(
            netuid,
            JungochainModule::get_current_block_as_u64()
        ));
        step_block(1);
...
pub fn step_rate_limit(transaction_type: &TransactionType, netuid: u16) {
    // Check rate limit
    let limit = JungochainModule::get_rate_limit(transaction_type, netuid);
...
pub fn unid(x: u16) -> u16 {
    ...
    pallet_jungochain::FirstReservedNetuids::<Test>::get() + x
}
