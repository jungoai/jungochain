...
fn test_serving_subscribe_ok_dispatch_info_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::serve_axon {
            netuid,
            version,
            ip,
            port,
...
fn test_serving_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::serve_axon(
        ...
        let neuron = JungochainModule::get_axon_info(netuid, &hotkey_account_id);
...
fn test_serving_tls_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, tempo, modality);
        register_ok_neuron(netuid, hotkey_account_id, U256::from(66), 0);
        assert_ok!(JungochainModule::serve_axon_tls(
            <<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id),
            netuid,
            version,
            ip,
...
fn test_prometheus_serving_subscribe_ok_dispatch_info_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let port: u16 = 128;
        let ip_type: u8 = 4;
        let call = RuntimeCall::JungochainModule(JungochainCall::serve_prometheus {
            netuid,
            version,
            ip,
            port,
...
fn test_prometheus_serving_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, tempo, modality);
        register_ok_neuron(netuid, hotkey_account_id, U256::from(66), 0);
        assert_ok!(JungochainModule::serve_prometheus(
            <<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id),
            netuid,
            version,
            ip,
        ...
        let neuron = JungochainModule::get_prometheus_info(netuid, &hotkey_account_id);
        assert_eq!(neuron.ip, ip);
        assert_eq!(neuron.version, version);
        assert_eq!(neuron.port, port);
        assert_eq!(neuron.ip_type, ip_type);
...
fn test_prometheus_serving_rate_limit_exceeded() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_serving_rate_limit(netuid, 1);
        // Same block, need 1 block to pass
        assert_eq!(
            JungochainModule::serve_prometheus(
                <<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id),
                netuid,
                version,
                ip,
...
fn test_serving_is_valid_ip_type_ok_ipv4() {
    new_test_ext(1).execute_with(|| {
        assert!(JungochainModule::is_valid_ip_type(4));
    });
}
...
fn test_serving_is_valid_ip_type_ok_ipv6() {
    new_test_ext(1).execute_with(|| {
        assert!(JungochainModule::is_valid_ip_type(6));
    });
}
...
fn test_serving_is_valid_ip_type_nok() {
    new_test_ext(1).execute_with(|| {
        assert!(!JungochainModule::is_valid_ip_type(10));
    });
}
...
fn test_serving_is_valid_ip_address_ipv4() {
    new_test_ext(1).execute_with(|| {
        assert!(JungochainModule::is_valid_ip_address(
            4,
            test::ipv4(8, 8, 8, 8)
        ));
    });
...
fn test_do_set_identity() {
    new_test_ext(1).execute_with(|| {
        ...
        // Set identity
        assert_ok!(JungochainModule::do_set_identity(
        ...
        assert_noop!(
            JungochainModule::do_set_identity(
...
fn test_is_valid_identity() {
    new_test_ext(1).execute_with(|| {
        ...
        assert!(JungochainModule::is_valid_identity(&valid_identity));
        ...
        assert!(JungochainModule::is_valid_identity(&max_length_identity));
        ...
        assert!(!JungochainModule::is_valid_identity(
        ...
        assert!(JungochainModule::is_valid_identity(&empty_identity));
        ...
        assert!(JungochainModule::is_valid_identity(&mixed_identity));
        ...
        assert!(JungochainModule::is_valid_identity(&max_field_identity));
    });
}
...
fn test_migrate_set_hotkey_identities() {
    new_test_ext(1).execute_with(|| {
        // Run the migration
        let weight =
            pallet_jungochain::migrations::migrate_chain_identity::migrate_set_hotkey_identities::<
                Test,
            >();
...
fn test_do_set_subnet_identity() {
    new_test_ext(1).execute_with(|| {
        ...
        // Set subnet identity
        assert_ok!(JungochainModule::do_set_subnet_identity(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey),
            netuid,
            subnet_name.clone(),
            github_repo.clone(),
        ...
        assert_noop!(
            JungochainModule::do_set_subnet_identity(
...
fn test_is_valid_subnet_identity() {
    new_test_ext(1).execute_with(|| {
        ...
        assert!(JungochainModule::is_valid_subnet_identity(&valid_identity));
        ...
        assert!(JungochainModule::is_valid_subnet_identity(
            &max_length_identity
        ));
        ...
        assert!(JungochainModule::is_valid_subnet_identity(&empty_identity));
        ...
        assert!(JungochainModule::is_valid_subnet_identity(&mixed_identity));
    });
}
...
fn test_set_subnet_identity_dispatch_info_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        let call: RuntimeCall =
            RuntimeCall::JungochainModule(JungochainCall::set_subnet_identity {
                netuid,
                subnet_name,
                github_repo,
                subnet_contact,
