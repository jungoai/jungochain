...
use pallet_jungochain::epoch::math::safe_exp;
...
fn uid_stats(netuid: u16, uid: u16) {
    log::info!(
        "stake: {:?}",
        JungochainModule::get_total_stake_for_hotkey(&(U256::from(uid)))
    );
    log::info!(
        "rank: {:?}",
        JungochainModule::get_rank_for_uid(netuid, uid)
    );
    log::info!(
        "trust: {:?}",
        JungochainModule::get_trust_for_uid(netuid, uid)
    );
    log::info!(
        "consensus: {:?}",
        JungochainModule::get_consensus_for_uid(netuid, uid)
    );
    log::info!(
        "incentive: {:?}",
        JungochainModule::get_incentive_for_uid(netuid, uid)
    );
    log::info!(
        "dividend: {:?}",
        JungochainModule::get_dividends_for_uid(netuid, uid)
    );
    log::info!(
        "emission: {:?}",
        JungochainModule::get_emission_for_uid(netuid, uid)
    );
}
...
fn init_run_epochs(
    ...
    // === Register uids
    JungochainModule::set_max_allowed_uids(netuid, n);
    for key in 0..n {
        ...
        JungochainModule::add_balance_to_coldkey_account(&(U256::from(key)), stake);
        JungochainModule::append_neuron(netuid, &(U256::from(key)), 0);
    ...
    assert_eq!(JungochainModule::get_subnetwork_n(netuid), n);

    // === Issue validator permits
    JungochainModule::set_max_allowed_validators(netuid, validators.len() as u16);
    assert_eq!(
        JungochainModule::get_max_allowed_validators(netuid),
        validators.len() as u16
    );
    JungochainModule::epoch(netuid, 1_000_000_000); // run first epoch to set allowed validators
    ...
    for _ in 0..epochs {
        if sparse {
            JungochainModule::epoch(netuid, 1_000_000_000);
        } else {
            JungochainModule::epoch_dense(netuid, 1_000_000_000);
        }
    }
    ...
    // let bonds = JungochainModule::get_bonds( netuid );
...
//                 for uid in set {
//                     major_emission +=
//                         I64F64::from_num(JungochainModule::get_emission_for_uid(netuid, uid));
//                 }
//             }
...
//         JungochainModule::set_max_allowed_uids(netuid, 3);
//         JungochainModule::increase_stake_on_coldkey_hotkey_account(
//             &U256::from(0),
//             &U256::from(0),
//             10,
...
//             10,
//         );
//         JungochainModule::append_neuron(netuid, &U256::from(0), 0);
//         JungochainModule::append_neuron(netuid, &U256::from(1), 0);
//         JungochainModule::append_neuron(netuid, &U256::from(2), 0);
//         JungochainModule::set_validator_permit_for_uid(0, 0, true);
//         JungochainModule::set_validator_permit_for_uid(0, 1, true);
//         JungochainModule::set_validator_permit_for_uid(0, 2, true);
//         assert_ok!(JungochainModule::set_weights(
...
//             0
//         ));
//         JungochainModule::epoch(0, u64::MAX);
//     });
// }

// Test an epoch on an empty graph.
// #[test]
// fn test_nill_epoch_jungochain() {
//     new_test_ext(1).execute_with(|| {
//         log::info!("test_nill_epoch:");
//         JungochainModule::epoch(0, 0);
//     });
// }
...
fn test_1_graph() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, u16::MAX - 1, 0); // set higher tempo to avoid built-in epoch, then manual epoch instead
        JungochainModule::set_max_allowed_uids(netuid, 1);
        JungochainModule::add_balance_to_coldkey_account(&coldkey, stake_amount);
        JungochainModule::increase_stake_on_coldkey_hotkey_account(&coldkey, &hotkey, stake_amount);
        JungochainModule::append_neuron(netuid, &hotkey, 0);
        ...
        // JungochainModule::set_weights_for_testing( netuid, i as u16, vec![ ( 0, u16::MAX )]); // doesn't set update status
        // JungochainModule::set_bonds_for_testing( netuid, uid, vec![ ( 0, u16::MAX )]); // rather, bonds are calculated in epoch
        JungochainModule::set_emission_values(&[netuid], vec![1_000_000_000]).unwrap();
        assert_eq!(
        ...
        JungochainModule::epoch(netuid, 1_000_000_000);
        assert_eq!(
            JungochainModule::get_total_stake_for_hotkey(&hotkey),
            stake_amount
        );
        assert_eq!(JungochainModule::get_rank_for_uid(netuid, uid), 0);
        assert_eq!(JungochainModule::get_trust_for_uid(netuid, uid), 0);
        assert_eq!(JungochainModule::get_consensus_for_uid(netuid, uid), 0);
        assert_eq!(JungochainModule::get_incentive_for_uid(netuid, uid), 0);
        assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 0);
        assert_eq!(
            JungochainModule::get_emission_for_uid(netuid, uid),
            1_000_000_000
        );
    ...
    new_test_ext(1).execute_with(|| {
        ...
        pub fn add_node(netuid: u16, coldkey: U256, hotkey: U256, uid: u16, stake_amount: u64) {
            log::info!(
                ...
                uid,
                stake_amount,
                JungochainModule::get_subnetwork_n(netuid),
            );
            JungochainModule::increase_stake_on_coldkey_hotkey_account(
                &coldkey,
                &hotkey,
            ...
            JungochainModule::append_neuron(netuid, &hotkey, 0);
            assert_eq!(JungochainModule::get_subnetwork_n(netuid) - 1, uid);
        }
        ...
        let netuid: u16 = 1;
        add_network(netuid, u16::MAX - 1, 0); // set higher tempo to avoid built-in epoch, then manual epoch instead
        JungochainModule::set_max_allowed_uids(netuid, n as u16);
        for i in 0..10 {
            add_node(netuid, U256::from(i), U256::from(i), i as u16, 1)
        }
        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 10);
        run_to_block(1); // run to next block to ensure weights are set on nodes after their registration block
        for i in 0..10 {
            assert_ok!(JungochainModule::set_weights(
        ...
        for i in 0..n {
            assert_eq!(
                JungochainModule::get_total_stake_for_hotkey(&(U256::from(i))),
                1
            );
            assert_eq!(JungochainModule::get_rank_for_uid(netuid, i as u16), 0);
            assert_eq!(JungochainModule::get_trust_for_uid(netuid, i as u16), 0);
            assert_eq!(JungochainModule::get_consensus_for_uid(netuid, i as u16), 0);
            assert_eq!(JungochainModule::get_incentive_for_uid(netuid, i as u16), 0);
            assert_eq!(JungochainModule::get_dividends_for_uid(netuid, i as u16), 0);
            assert_eq!(
                JungochainModule::get_emission_for_uid(netuid, i as u16),
                99999999
            );
        }
    });
        ...
        for server_self in [false, true] {
            ...
            new_test_ext(1).execute_with(|| {
                ...
                for uid in validators {
                    ...
                    assert_eq!(JungochainModule::get_consensus_for_uid(netuid, uid), 0);
                    assert_eq!(JungochainModule::get_incentive_for_uid(netuid, uid), 0);
                    assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 1023); // Note D = floor(1 / 64 * 65_535) = 1023
                    assert_eq!(JungochainModule::get_emission_for_uid(netuid, uid), 7812500); // Note E = 0.5 / 200 * 1_000_000_000 = 7_812_500
                ...
                for uid in servers {
                    ...
                    assert_eq!(JungochainModule::get_rank_for_uid(netuid, uid), 146); // Note R = floor(1 / (512 - 64) * 65_535) = 146
                    assert_eq!(JungochainModule::get_trust_for_uid(netuid, uid), 65535);
                    assert_eq!(JungochainModule::get_consensus_for_uid(netuid, uid), 146); // Note C = floor(1 / (512 - 64) * 65_535) = 146
                    assert_eq!(JungochainModule::get_incentive_for_uid(netuid, uid), 146); // Note I = floor(1 / (512 - 64) * 65_535) = 146
                    assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 0);
                    assert_eq!(JungochainModule::get_emission_for_uid(netuid, uid), 1116071); // Note E = floor(0.5 / (512 - 64) * 1_000_000_000) = 1_116_071
                    assert_eq!(bonds[uid as usize][validator], 0.0);
    ...
    for interleave in 0..3 {
        for server_self in [false, true] {
            ...
            new_test_ext(1).execute_with(|| {
                ...
                let bond = JungochainModule::get_bonds(netuid);
                for uid in 0..network_n {
                    rank.push(JungochainModule::get_rank_for_uid(netuid, uid));
                    incentive.push(JungochainModule::get_incentive_for_uid(netuid, uid));
                    dividend.push(JungochainModule::get_dividends_for_uid(netuid, uid));
                    emission.push(JungochainModule::get_emission_for_uid(netuid, uid));
    ...
    for interleave in 0..3 {
        ...
        for server_self in [false, true] {
            ...
            new_test_ext(1).execute_with(|| {
                ...
                assert_eq!(JungochainModule::get_total_stake(), 21_000_000_000_000_000);
                let bonds = JungochainModule::get_bonds(netuid);
                for uid in &validators {
                    assert_eq!(
                        JungochainModule::get_total_stake_for_hotkey(&(U256::from(*uid as u64))),
                        max_stake_per_validator
                    );
                    assert_eq!(JungochainModule::get_rank_for_uid(netuid, *uid), 0);
                    assert_eq!(JungochainModule::get_trust_for_uid(netuid, *uid), 0);
                    assert_eq!(JungochainModule::get_consensus_for_uid(netuid, *uid), 0);
                    assert_eq!(JungochainModule::get_incentive_for_uid(netuid, *uid), 0);
                    assert_eq!(JungochainModule::get_dividends_for_uid(netuid, *uid), 255); // Note D = floor(1 / 256 * 65_535)
                    assert_eq!(
                        JungochainModule::get_emission_for_uid(netuid, *uid),
                        1953125
                    ); // Note E = 0.5 / 256 * 1_000_000_000 = 1953125
                    assert_eq!(bonds[*uid as usize][validator], 0.0);
                    assert_eq!(
                ...
                for uid in &servers {
                    ...
                    assert_eq!(JungochainModule::get_rank_for_uid(netuid, *uid), 17); // Note R = floor(1 / (4096 - 256) * 65_535) = 17
                    assert_eq!(JungochainModule::get_trust_for_uid(netuid, *uid), 65535);
                    assert_eq!(JungochainModule::get_consensus_for_uid(netuid, *uid), 17); // Note C = floor(1 / (4096 - 256) * 65_535) = 17
                    assert_eq!(JungochainModule::get_incentive_for_uid(netuid, *uid), 17); // Note I = floor(1 / (4096 - 256) * 65_535) = 17
                    assert_eq!(JungochainModule::get_dividends_for_uid(netuid, *uid), 0);
                    assert_eq!(JungochainModule::get_emission_for_uid(netuid, *uid), 130208); // Note E = floor(0.5 / (4096 - 256) * 1_000_000_000) = 130208
                    assert_eq!(bonds[*uid as usize][validator], 0.0);
                    assert_eq!(bonds[*uid as usize][server], 0.0);
                }
            });
...
fn test_16384_graph_sparse() {
    new_test_ext(1).execute_with(|| {
        ...
        for uid in validators {
            ...
            assert_eq!(JungochainModule::get_rank_for_uid(netuid, uid), 0);
            assert_eq!(JungochainModule::get_trust_for_uid(netuid, uid), 0);
            assert_eq!(JungochainModule::get_consensus_for_uid(netuid, uid), 438); // Note C = 0.0066928507 = (0.0066928507*65_535) = floor( 438.6159706245 )
            assert_eq!(JungochainModule::get_incentive_for_uid(netuid, uid), 0);
            assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 127); // Note D = floor(1 / 512 * 65_535) = 127
            assert_eq!(JungochainModule::get_emission_for_uid(netuid, uid), 976085); // Note E = 0.5 / 512 * 1_000_000_000 = 976_562 (discrepancy)
        ...
        for uid in servers {
            ...
            assert_eq!(JungochainModule::get_rank_for_uid(netuid, uid), 4); // Note R = floor(1 / (16384 - 512) * 65_535) = 4
            assert_eq!(JungochainModule::get_trust_for_uid(netuid, uid), 65535);
            assert_eq!(JungochainModule::get_consensus_for_uid(netuid, uid), 4); // Note C = floor(1 / (16384 - 512) * 65_535) = 4
            assert_eq!(JungochainModule::get_incentive_for_uid(netuid, uid), 4); // Note I = floor(1 / (16384 - 512) * 65_535) = 4
            assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 0);
            assert_eq!(JungochainModule::get_emission_for_uid(netuid, uid), 31517); // Note E = floor(0.5 / (16384 - 512) * 1_000_000_000) = 31502 (discrepancy)
            assert_eq!(bonds[uid as usize][0], 0.0);
...
fn test_bonds() {
    new_test_ext(1).execute_with(|| {
		...
		JungochainModule::set_max_allowed_uids( netuid, n );
		assert_eq!(JungochainModule::get_max_allowed_uids(netuid), n);
		JungochainModule::set_max_registrations_per_block( netuid, n );
		JungochainModule::set_target_registrations_per_interval(netuid, n);
		JungochainModule::set_weights_set_rate_limit( netuid, 0 );
        JungochainModule::set_min_allowed_weights( netuid, 1 );
        JungochainModule::set_max_weight_limit( netuid, u16::MAX );

		// === Register [validator1, validator2, validator3, validator4, server1, server2, server3, server4]
		for key in 0..n as u64 {
			JungochainModule::add_balance_to_coldkey_account( &U256::from(key), max_stake );
			let (nonce, work): (u64, Vec<u8>) = JungochainModule::create_work_for_block_number( netuid, block_number, key * 1_000_000, &U256::from(key));
			assert_ok!(JungochainModule::register(<<Test as Config>::RuntimeOrigin>::signed(U256::from(key)), netuid, block_number, nonce, work, U256::from(key), U256::from(key)));
			JungochainModule::increase_stake_on_coldkey_hotkey_account( &U256::from(key), &U256::from(key), stakes[key as usize] );
		}
		assert_eq!(JungochainModule::get_max_allowed_uids(netuid), n);
		assert_eq!(JungochainModule::get_subnetwork_n(netuid), n);

		// === Issue validator permits
		JungochainModule::set_max_allowed_validators(netuid, n);
		assert_eq!( JungochainModule::get_max_allowed_validators(netuid), n);
		JungochainModule::epoch( netuid, 1_000_000_000 ); // run first epoch to set allowed validators
		...
		for uid in 0..(n/2) as u64 {
			assert_ok!(JungochainModule::set_weights(RuntimeOrigin::signed(U256::from(uid)), netuid, ((n/2)..n).collect(), vec![ u16::MAX/4, u16::MAX/2, (u16::MAX/4)*3, u16::MAX], 0));
		}
		if sparse { JungochainModule::epoch( netuid, 1_000_000_000 ); }
		else { JungochainModule::epoch_dense( netuid, 1_000_000_000 ); }
		/*  n: 8
		...
		let uid = 0;
		assert_ok!(JungochainModule::set_weights(RuntimeOrigin::signed(U256::from(uid)), netuid, vec![uid], vec![u16::MAX], 0));
        next_block();
		if sparse { JungochainModule::epoch( netuid, 1_000_000_000 ); }
		else { JungochainModule::epoch_dense( netuid, 1_000_000_000 ); }
		/*  n: 8
		...
		// === Set val3->srv4: 1
		assert_ok!(JungochainModule::set_weights(RuntimeOrigin::signed(U256::from(2)), netuid, vec![7], vec![u16::MAX], 0));
        next_block();
		if sparse { JungochainModule::epoch( netuid, 1_000_000_000 ); }
		else { JungochainModule::epoch_dense( netuid, 1_000_000_000 ); }
		/*  current_block: 5
...
fn test_bonds_with_liquid_alpha() {
    new_test_ext(1).execute_with(|| {
        ...
        // Set weights
        for uid in 0..(n / 2) {
            JungochainModule::set_validator_permit_for_uid(netuid, uid, true);
            assert_ok!(JungochainModule::set_weights(
                RuntimeOrigin::signed(U256::from(uid)),
                netuid,
                ((n / 2)..n).collect(),
        ...
        // Enable Liquid Alpha
        JungochainModule::set_liquid_alpha_enabled(netuid, true);
...
fn test_set_alpha_disabled() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::add_balance_to_coldkey_account(&coldkey, 1_000_000_000_000_000);
        assert_ok!(JungochainModule::root_register(signer.clone(), hotkey,));
        assert_ok!(JungochainModule::add_stake(signer.clone(), hotkey, 1000));
        // Only owner can set alpha values
        assert_ok!(JungochainModule::register_network(signer.clone()));

        // Explicitly set to false
        JungochainModule::set_liquid_alpha_enabled(netuid, false);
        assert_err!(
            JungochainModule::do_set_alpha_values(signer.clone(), netuid, 12_u16, u16::MAX),
            Error::<Test>::LiquidAlphaDisabled
        );

        JungochainModule::set_liquid_alpha_enabled(netuid, true);
        assert_ok!(JungochainModule::do_set_alpha_values(
            signer.clone(),
            netuid,
            12_u16,
            u16::MAX
...
fn test_active_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_max_registrations_per_block(netuid, n);
        JungochainModule::set_target_registrations_per_interval(netuid, n);
        JungochainModule::set_min_allowed_weights(netuid, 0);
        JungochainModule::set_max_weight_limit(netuid, u16::MAX);
        ...
        for uid in 0..n {
            ...
            if uid < n / 2 {
                assert_eq!(JungochainModule::get_dividends_for_uid(netuid, uid), 32767);
        ...
        let activity_cutoff: u64 = JungochainModule::get_activity_cutoff(netuid) as u64;
        ...
        let bonds = JungochainModule::get_bonds(netuid);
        assert_eq!(JungochainModule::get_dividends_for_uid(netuid, 0), 36044); // Note D = floor((0.5 * 0.9 + 0.1) * 65_535)
        assert_eq!(JungochainModule::get_emission_for_uid(netuid, 0), 274999999); // Note E = 0.5 * 0.55 * 1_000_000_000 = 275_000_000 (discrepancy)
        ...
        for validator in 1..(n / 2) {
            assert_eq!(
                JungochainModule::get_dividends_for_uid(netuid, validator),
                29490
            ); // Note D = floor((0.5 * 0.9) * 65_535)
            assert_eq!(
                JungochainModule::get_emission_for_uid(netuid, validator),
                224999999
        ...
        let bonds = JungochainModule::get_bonds(netuid);
        assert_eq!(JungochainModule::get_dividends_for_uid(netuid, 0), 35716); // Note D = floor((0.55 * 0.9 + 0.5 * 0.1) * 65_535)
        assert_eq!(JungochainModule::get_emission_for_uid(netuid, 0), 272501132); // Note E = 0.5 * (0.55 * 0.9 + 0.5 * 0.1) * 1_000_000_000 = 272_500_000 (discrepancy)
        for server in ((n / 2) as usize)..n as usize {
            assert_eq!(bonds[0][server], I32F32::from_num(65_535)); // floor((0.55 * 0.9 + 0.5 * 0.1)*(2^16-1))/(2^16-1), then max-upscale
        }
        assert_eq!(JungochainModule::get_dividends_for_uid(netuid, 1), 29818); // Note D = floor((0.45 * 0.9 + 0.5 * 0.1) * 65_535)
        assert_eq!(JungochainModule::get_emission_for_uid(netuid, 1), 227498866); // Note E = 0.5 * (0.45 * 0.9 + 0.5 * 0.1) * 1_000_000_000 = 227_500_000 (discrepancy)
        for server in ((n / 2) as usize)..n as usize {
...
fn test_outdated_weights() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_min_allowed_weights(netuid, 0);
        JungochainModule::set_max_weight_limit(netuid, u16::MAX);
        assert_eq!(JungochainModule::get_registrations_this_block(netuid), 0);
        ...
        assert_eq!(JungochainModule::get_max_registrations_per_block(netuid), n);
        assert_eq!(JungochainModule::get_registrations_this_block(netuid), 0);
        assert_ok!(JungochainModule::register(
        ...
        assert_eq!(
            U256::from(new_key),
            JungochainModule::get_hotkey_for_net_and_uid(netuid, deregistered_uid)
        ...
        let bonds = JungochainModule::get_bonds(netuid);
        assert_eq!(JungochainModule::get_dividends_for_uid(netuid, 0), 32767); // Note D = floor(0.5 * 65_535)
        assert_eq!(JungochainModule::get_emission_for_uid(netuid, 0), 250000000); // Note E = 0.5 * 0.5 * 1_000_000_000 = 249311245
...
fn test_zero_weights() {
    new_test_ext(1).execute_with(|| {
        ...
        for validator in 0..(n / 2) as u64 {
            JungochainModule::add_balance_to_coldkey_account(&U256::from(validator), stake);
            JungochainModule::increase_stake_on_coldkey_hotkey_account(
                &U256::from(validator),
                &U256::from(validator),
                stake,
        ...
        for server in (n / 2)..n {
            assert_eq!(JungochainModule::get_emission_for_uid(netuid, server), 0);
            // no stake
        }
        run_to_block(1);
        ...
        for (network_n, validators_n) in [(2, 1), (4, 2), (8, 4)] {
            for assignment in 0..=1 {
                ...
                new_test_ext(1).execute_with(|| {
                    ...
                    add_network(netuid, tempo, 0);
                    JungochainModule::set_max_allowed_uids(netuid, network_n as u16);
                    assert_eq!(
                        JungochainModule::get_max_allowed_uids(netuid),
                        network_n as u16
                    );
                    JungochainModule::set_max_registrations_per_block(netuid, network_n as u16);
                    ...
                    for key in 0..network_n as u64 {
                        ...
                        let (nonce, work): (u64, Vec<u8>) =
                            JungochainModule::create_work_for_block_number(
                                netuid,
                                block_number,
                                key * 1_000_000,
                                &U256::from(key),
                    ...
                    assert_eq!(JungochainModule::get_subnetwork_n(netuid), network_n as u16);

                    // === Issue validator permits
                    JungochainModule::set_max_allowed_validators(netuid, validators_n as u16);
                    assert_eq!(
                        JungochainModule::get_max_allowed_validators(netuid),
                        validators_n as u16
                    );
                    ...
                    for validator in &validators {
                        assert_eq!(
                            correct,
                            JungochainModule::get_validator_permit_for_uid(netuid, *validator)
                        );
                    }
                    for server in &servers {
                        assert_eq!(
                            !correct,
                            JungochainModule::get_validator_permit_for_uid(netuid, *server)
                        );
                    }
...
fn test_compute_alpha_values() {
    ...
    let alpha = JungochainModule::compute_alpha_values(&consensus, a, b);
...
fn test_clamp_alpha_values() {
    ...
    let clamped_alpha = JungochainModule::clamp_alpha_values(alpha.clone(), alpha_high, alpha_low);
...
fn test_calculate_logistic_params() {
    ...
    // Call the function
    let (a, b) = JungochainModule::calculate_logistic_params(
        alpha_high,
        alpha_low,
        consensus_high,
        consensus_low,
...
fn test_compute_ema_bonds_with_liquid_alpha_sparse() {
    ...
    // Call the function
    let ema_bonds =
        JungochainModule::compute_ema_bonds_with_liquid_alpha_sparse(&bonds_delta, &bonds, alpha);
...
fn test_get_set_alpha() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_err!(
            JungochainModule::do_set_alpha_values(signer.clone(), netuid, alpha_low, alpha_high),
        ...
        let (grabbed_alpha_low, grabbed_alpha_high): (u16, u16) =
            JungochainModule::get_alpha_values(netuid);
        ...
        let (alpha_low_32, alpha_high_32) = JungochainModule::get_alpha_values_32(netuid);
        ...
        JungochainModule::set_liquid_alpha_enabled(netuid, true); // Re-enable for further tests
        assert_ok!(JungochainModule::do_set_alpha_values(
            signer.clone(),
            netuid,
            alpha_low,
        ...
        assert_err!(
            JungochainModule::do_set_alpha_values(
                signer.clone(),
                netuid,
                alpha_low,
                alpha_high_too_low
...
fn test_blocks_since_last_step() {
    new_test_ext(1).execute_with(|| {
        ...
        let original_blocks: u64 = JungochainModule::get_blocks_since_last_step(netuid);

        step_block(5);

        let new_blocks: u64 = JungochainModule::get_blocks_since_last_step(netuid);

        assert!(new_blocks > original_blocks);
        assert_eq!(new_blocks, 5);

        let blocks_to_step: u16 = JungochainModule::blocks_until_next_epoch(
            netuid,
            tempo,
        ...
        let post_blocks: u64 = JungochainModule::get_blocks_since_last_step(netuid);
        ...
        let new_post_blocks: u64 = JungochainModule::get_blocks_since_last_step(netuid);

        assert_eq!(new_post_blocks, 20);

        step_block(7);

        assert_eq!(JungochainModule::get_blocks_since_last_step(netuid), 27);
    });
}
...
// 					for set in vec![major_validators, major_servers] {
// 						for uid in set {
// 							major_emission += I64F64::from_num(JungochainModule::get_emission_for_uid( netuid, uid ));
// 						}
// 					}
// 					for set in vec![minor_validators, minor_servers] {
// 						for uid in set {
// 							minor_emission += I64F64::from_num(JungochainModule::get_emission_for_uid( netuid, uid ));
// 						}
// 					}
