...
use mock::*;
use pallet_jungochain::{Error, Owner};
...
// Test the call passes through the jungochain module.
#[test]
fn test_set_weights_dispatch_info_ok() {
    new_test_ext(0).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::set_weights {
            netuid,
            dests,
            weights,
            version_key,
...
fn test_set_rootweights_dispatch_info_ok() {
    new_test_ext(0).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::set_root_weights {
            dests,
            weights,
            version_key,
...
fn test_set_rootweights_validate() {
    ...
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_weights_min_stake(min_stake);

        // Verify stake is less than minimum
        assert!(JungochainModule::get_total_stake_for_hotkey(&hotkey) < min_stake);
        let info: DispatchInfo =
            DispatchInfoOf::<<Test as frame_system::Config>::RuntimeCall>::default();

        let extension = pallet_jungochain::JungochainSignedExtension::<Test>::new();
        ...
        JungochainModule::increase_stake_on_hotkey_account(&hotkey, min_stake);
        ...
        JungochainModule::increase_stake_on_hotkey_account(&hotkey, 1);
...
fn test_commit_weights_dispatch_info_ok() {
    new_test_ext(0).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::commit_weights {
            netuid,
            commit_hash,
        });
        let dispatch_info = call.get_dispatch_info();
...
fn test_reveal_weights_dispatch_info_ok() {
    new_test_ext(0).execute_with(|| {
        ...
        let call = RuntimeCall::JungochainModule(JungochainCall::reveal_weights {
            netuid,
            uids: dests,
            values: weights,
            salt,
...
fn test_weights_err_no_validator_permit() {
    new_test_ext(0).execute_with(|| {
        ...
        let result = JungochainModule::set_weights(
        ...
        let weight_values: Vec<u16> = vec![1, 2];
        let neuron_uid: u16 =
            JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey_account_id)
                .expect("Not registered.");
        JungochainModule::set_validator_permit_for_uid(netuid, neuron_uid, true);
        let result = JungochainModule::set_weights(
            RuntimeOrigin::signed(hotkey_account_id),
            netuid,
            weights_keys,
...
// To execute this test: cargo test --package pallet-jungochain --test weights test_set_weights_min_stake_failed -- --nocapture`
#[test]
fn test_set_weights_min_stake_failed() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_weights_min_stake(20_000_000_000_000);
        ...
        JungochainModule::increase_stake_on_hotkey_account(&hotkey, 19_000_000_000_000);
        assert!(!JungochainModule::check_weights_min_stake(&hotkey, netuid));
        JungochainModule::increase_stake_on_hotkey_account(&hotkey, 20_000_000_000_000);
        assert!(JungochainModule::check_weights_min_stake(&hotkey, netuid));

        // Check that it fails at the pallet level.
        JungochainModule::set_weights_min_stake(100_000_000_000_000);
        assert_eq!(
            JungochainModule::set_weights(
                RuntimeOrigin::signed(hotkey),
                netuid,
        ...
        // Now passes
        JungochainModule::increase_stake_on_hotkey_account(&hotkey, 100_000_000_000_000);
...
fn test_weights_version_key() {
    new_test_ext(0).execute_with(|| {
        ...
        let key0: u64 = 12312;
        let key1: u64 = 20313;
        JungochainModule::set_weights_version_key(netuid0, key0);
        JungochainModule::set_weights_version_key(netuid1, key1);
    ...
    new_test_ext(0).execute_with(|| {
        ...
        let neuron_uid: u16 =
            JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey_account_id)
                .expect("Not registered.");
        JungochainModule::set_validator_permit_for_uid(netuid, neuron_uid, true);
        JungochainModule::set_weights_set_rate_limit(netuid, 10);
        assert_eq!(JungochainModule::get_weights_set_rate_limit(netuid), 10);
...
fn test_weights_err_has_duplicate_ids() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_max_allowed_uids(netuid, 100); // Allow many registrations per block.
        JungochainModule::set_max_registrations_per_block(netuid, 100); // Allow many registrations per block.
        JungochainModule::set_target_registrations_per_interval(netuid, 100); // Allow many registrations per block.
                                                                              // uid 0
        ...
        JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(1))
            .expect("Not registered.");

        // uid 2
        register_ok_neuron(netuid, U256::from(2), U256::from(1), 200_000);
        JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(2))
            .expect("Not registered.");

        // uid 3
        register_ok_neuron(netuid, U256::from(3), U256::from(1), 300_000);
        JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(3))
            .expect("Not registered.");

        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 4);
...
fn test_weights_err_max_weight_limit() {
    ...
    new_test_ext(0).execute_with(|| {
        ...
        // Set params.
        JungochainModule::set_max_allowed_uids(netuid, 5);
        JungochainModule::set_target_registrations_per_interval(netuid, 5);
        JungochainModule::set_max_weight_limit(netuid, u16::MAX / 5);
        JungochainModule::set_min_allowed_weights(netuid, 0);

        // Add 5 accounts.
        println!("+Registering: net:{:?}, cold:{:?}, hot:{:?}", netuid, 0, 0);
        register_ok_neuron(netuid, U256::from(0), U256::from(0), 55555);
        let neuron_uid: u16 = JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(0))
            .expect("Not registered.");
        JungochainModule::set_validator_permit_for_uid(netuid, neuron_uid, true);
        ...
        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 2);
        step_block(1);
        ...
        assert_eq!(JungochainModule::get_subnetwork_n(netuid), 5);
        step_block(1);
        ...
        let result =
            JungochainModule::set_weights(RuntimeOrigin::signed(U256::from(0)), 1, uids, values, 0);
...
fn test_no_signature() {
    new_test_ext(0).execute_with(|| {
        ...
        let result = JungochainModule::set_weights(RuntimeOrigin::none(), 1, uids, values, 0);
...
fn test_set_weights_err_not_active() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(666))
            .expect("Not registered.");
...
fn test_set_weight_not_enough_values() {
    new_test_ext(0).execute_with(|| {
        ...
        let neuron_uid: u16 = JungochainModule::get_uid_for_net_and_hotkey(netuid, &U256::from(1))
            .expect("Not registered.");
        JungochainModule::set_validator_permit_for_uid(netuid, neuron_uid, true);
        JungochainModule::set_max_weight_limit(netuid, u16::MAX);

        register_ok_neuron(1, U256::from(3), U256::from(4), 300000);
        JungochainModule::set_min_allowed_weights(netuid, 2);
...
fn test_set_weight_too_many_uids() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_min_allowed_weights(1, 2);
        JungochainModule::set_max_weight_limit(netuid, u16::MAX);
...
fn test_set_weights_sum_larger_than_u16_max() {
    new_test_ext(0).execute_with(|| {
        ...
        let all_weights: Vec<Vec<I32F32>> = JungochainModule::get_weights(netuid);
...
fn test_check_length_allows_singleton() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_min_allowed_weights(netuid, min_allowed_weights);
        ...
        let expected = true;
        let result = JungochainModule::check_length(netuid, uid, &uids, &weights);
...
fn test_check_length_to_few_weights() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_target_registrations_per_interval(netuid, 100);
        JungochainModule::set_max_registrations_per_block(netuid, 100);
...
fn test_normalize_weights_does_not_mutate_when_sum_is_zero() {
    new_test_ext(0).execute_with(|| {
        ...
        let result = JungochainModule::normalize_weights(weights);
...
fn test_max_weight_limited_allow_self_weights_to_exceed_max_weight_limit() {
    new_test_ext(0).execute_with(|| {
        ...
        let expected = true;
        let result = JungochainModule::max_weight_limited(netuid, uid, &uids, &weights);
...
fn test_max_weight_limited_when_max_weight_is_within_limit() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_max_weight_limit(netuid, max_weight_limit);
...
fn test_is_self_weight_weights_length_not_one() {
    new_test_ext(0).execute_with(|| {
        ...
        let expected = false;
        let result = JungochainModule::is_self_weight(uid, &uids, &weights);
...
fn test_check_len_uids_within_allowed_within_network_pool() {
    new_test_ext(0).execute_with(|| {
        ...
        let max_allowed: u16 = JungochainModule::get_subnetwork_n(netuid);

        JungochainModule::set_max_allowed_uids(netuid, max_allowed);
        JungochainModule::set_max_registrations_per_block(netuid, max_registrations_per_block);

        let uids: Vec<u16> = Vec::from_iter(0..max_allowed);

        let expected = true;
        let result = JungochainModule::check_len_uids_within_allowed(netuid, &uids);
        assert_eq!(
            expected, result,
...
fn test_set_weights_commit_reveal_enabled_error() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_commit_reveal_weights_enabled(netuid, true);
        ...
        JungochainModule::set_commit_reveal_weights_enabled(netuid, false);
...
fn test_reveal_weights_when_commit_reveal_disabled() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_weights_set_rate_limit(netuid, 5);
        JungochainModule::set_validator_permit_for_uid(netuid, 0, true);
        JungochainModule::set_validator_permit_for_uid(netuid, 1, true);

        // Enable commit-reveal and commit
        JungochainModule::set_commit_reveal_weights_enabled(netuid, true);
        assert_ok!(JungochainModule::commit_weights(
            RuntimeOrigin::signed(hotkey),
            netuid,
            commit_hash
        ));
        ...
        assert_err!(
            JungochainModule::reveal_weights(
                RuntimeOrigin::signed(hotkey),
                netuid,
                uids,
                weight_values,
...
fn test_commit_reveal_weights_ok() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::reveal_weights(
            RuntimeOrigin::signed(hotkey),
            netuid,
            uids,
            weight_values,
...
fn test_commit_reveal_disabled_or_enabled() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_err!(
            JungochainModule::commit_weights(RuntimeOrigin::signed(hotkey), netuid, commit_hash),
            Error::<Test>::CommitRevealDisabled
        );
...
fn test_tempo_change_during_commit_reveal_process() {
    new_test_ext(0).execute_with(|| {
        ...
        JungochainModule::set_tempo(netuid, tempo_before_next_reveal);
        ...
        JungochainModule::set_tempo(netuid, tempo);
...
fn test_commit_reveal_multiple_commits() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_err!(
            JungochainModule::commit_weights(RuntimeOrigin::signed(hotkey), netuid, commit_hash_11),
        ...
        let remaining_commits = pallet_jungochain::WeightCommits::<Test>::get(netuid, hotkey)
...
fn test_expired_commits_handling_in_commit_and_reveal() {
    new_test_ext(1).execute_with(|| {
        ...
        let commits: VecDeque<(H256, u64, u64, u64)> =
            pallet_jungochain::WeightCommits::<Test>::get(netuid, hotkey)
        ...
        let commits = pallet_jungochain::WeightCommits::<Test>::get(netuid, hotkey);
...
fn test_reveal_at_exact_epoch() {
    new_test_ext(1).execute_with(|| {
        ...
        for &reveal_period in &reveal_periods {
            JungochainModule::set_reveal_period(netuid, reveal_period);
            ...
            let commit_block = JungochainModule::get_current_block_as_u64();
            let commit_epoch = JungochainModule::get_epoch_index(netuid, commit_block);
            ...
            let current_epoch = JungochainModule::get_epoch_index(
                netuid,
                JungochainModule::get_current_block_as_u64(),
            );
            ...
            pallet_jungochain::WeightCommits::<Test>::remove(netuid, hotkey);
        }
    });
}
...
fn test_tempo_and_reveal_period_change_during_commit_reveal_process() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_reveal_period(netuid, initial_reveal_period);
        JungochainModule::set_commit_reveal_weights_enabled(netuid, true);
        ...
        JungochainModule::set_tempo(netuid, new_tempo);
        JungochainModule::set_reveal_period(netuid, new_reveal_period);
        log::info!(
        ...
        let current_epoch = JungochainModule::get_epoch_index(netuid, current_block);
        ...
        JungochainModule::set_tempo(netuid, new_tempo_after_reveal);
        JungochainModule::set_reveal_period(netuid, new_reveal_period_after_reveal);
        log::info!(
        ...
        let new_commit_block = JungochainModule::get_current_block_as_u64();
        let new_commit_epoch = JungochainModule::get_epoch_index(netuid, new_commit_block);
...
fn test_successful_batch_reveal() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(JungochainModule::do_batch_reveal_weights(
            RuntimeOrigin::signed(hotkey),
            netuid,
            uids_list.clone(),
...
fn test_batch_reveal_with_expired_commits() {
    new_test_ext(1).execute_with(|| {
        ...
        let result = JungochainModule::do_batch_reveal_weights(
            RuntimeOrigin::signed(hotkey),
            netuid,
            uids_list.clone(),
            weight_values_list.clone(),
...
fn test_highly_concurrent_commits_and_reveals_with_multiple_hotkeys() {
    new_test_ext(1).execute_with(|| {
        ...
        JungochainModule::set_weights_set_rate_limit(netuid, 0);
        JungochainModule::set_reveal_period(netuid, initial_reveal_period);
        JungochainModule::set_max_registrations_per_block(netuid, u16::MAX);
        JungochainModule::set_target_registrations_per_interval(netuid, u16::MAX);
        ...
        for i in 0..commits_per_hotkey {
            for hotkey in &hotkeys {
                ...
                let current_commits = pallet_jungochain::WeightCommits::<Test>::get(netuid, hotkey)
                    .unwrap_or_default();
            ...
            for hotkey in &hotkeys {
                if let Some(commits) = commit_info_map.get_mut(hotkey) {
                    ...
                    let reveal_result = JungochainModule::reveal_weights(
                        RuntimeOrigin::signed(*hotkey),
                        netuid,
                        uids.clone(),
                        values.clone(),
        ...
        JungochainModule::set_tempo(netuid, 150);
        JungochainModule::set_reveal_period(netuid, 7);
        ...
        JungochainModule::set_tempo(netuid, 200);
        JungochainModule::set_reveal_period(netuid, 10);
        ...
        for hotkey in &hotkeys {
            ...
            assert_err!(
                JungochainModule::commit_weights(
                    RuntimeOrigin::signed(*hotkey),
                    netuid,
                    commit_hash
                ),
        ...
        assert_eq!(JungochainModule::get_reveal_period(netuid), 10);
        assert_eq!(JungochainModule::get_tempo(netuid), 200);
    })
}
...
fn test_get_reveal_blocks() {
    new_test_ext(1).execute_with(|| {
        ...
        let (first_reveal_block, last_reveal_block) =
            JungochainModule::get_reveal_blocks(netuid, 0);
        ...
        let result = JungochainModule::reveal_weights(
            RuntimeOrigin::signed(hotkey),
            netuid,
            uids.clone(),
            weight_values.clone(),
...
fn test_commit_weights_rate_limit() {
    new_test_ext(1).execute_with(|| {
        ...
        register_ok_neuron(netuid, U256::from(3), U256::from(4), 300_000);
        register_ok_neuron(netuid, U256::from(1), U256::from(2), 100_000);
        JungochainModule::set_weights_set_rate_limit(netuid, 10); // Rate limit is 10 blocks
        JungochainModule::set_validator_permit_for_uid(netuid, 0, true);
        JungochainModule::set_validator_permit_for_uid(netuid, 1, true);
        JungochainModule::set_commit_reveal_weights_enabled(netuid, true);

        let neuron_uid =
            JungochainModule::get_uid_for_net_and_hotkey(netuid, &hotkey).expect("expected uid");
        JungochainModule::set_last_update_for_uid(netuid, neuron_uid, 0);
